...
namespace Bee.Api.Core
{
    ...
    public class ApiPayloadTransformer : IApiPayloadTransformer
    {
        ...
        public object Encode(object payload, Type type, byte[] encryptionKey)
        {
            if (payload == null)
            {
            ...
            try
            {
                ...
                return ApiServiceOptions.PayloadEncryptor.Encrypt(compressedBytes, encryptionKey);  // 加密
            }
            catch (Exception ex)
            {
        ...
        public object Decode(object payload, Type type, byte[] encryptionKey)
        {
            if (payload == null)
            {
                throw new ArgumentNullException(nameof(payload), "Input data cannot be null.");
            ...
            try
            {
                ...
                byte[] decryptedBytes = ApiServiceOptions.PayloadEncryptor.Decrypt(bytes, encryptionKey);  // 解密
