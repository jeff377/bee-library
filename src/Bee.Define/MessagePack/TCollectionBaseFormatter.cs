...
{
    /// <summary>
    /// 用於序列化與反序列化繼承自 <see cref="TCollectionBase{T}"/> 的強型別集合格式化器。
    /// </summary>
    /// <typeparam name="TCollection">具體集合型別，需繼承自 <see cref="TCollectionBase{TElement}"/>，且具備無參數建構式。</typeparam>
    /// <typeparam name="TElement">集合成員型別。</typeparam>
    public class TCollectionBaseFormatter<TCollection, TElement> : IMessagePackFormatter<TCollection>
        where TCollection : TCollectionBase<TElement>, new()
