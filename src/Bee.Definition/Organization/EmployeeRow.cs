...
{
    /// <summary>
    /// A flat employee row as read from <c>ft_employee</c> (a company-database table), carrying the
    /// links needed to resolve the current user's department: the account link (<c>user_rowid</c>)
    /// and the department (<c>dept_rowid</c>).
    /// </summary>
    /// <param name="RowId">The employee row id (<c>ft_employee.sys_rowid</c>).</param>
