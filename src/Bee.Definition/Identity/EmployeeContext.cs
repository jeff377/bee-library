namespace Bee.Definition.Identity
{
    ...
    /// <param name="EmployeeRowId">The linked employee row id (<c>ft_employee.sys_rowid</c>); <see cref="System.Guid.Empty"/> when the user has no employee in this company. Used by the <c>Own</c> scope.</param>
    /// <param name="DeptRowId">The employee's department row id (<c>ft_employee.dept_rowid</c>); <see cref="System.Guid.Empty"/> when the user has no employee or no department. Used by the <c>Dept</c> / <c>DeptAndSub</c> scopes.</param>
