...
{
    /// <summary>
    /// Anomaly entry for the data-access layer (row in <c>st_log_anomaly_db</c>). Records which
    /// database and command hit an anomaly, from the technical perspective. <c>DbAccess</c> has no
    /// session context, so the common who / company columns are intentionally omitted.
    /// </summary>
    public sealed class DbAnomalyEntry : AnomalyEntry
    {
        /// <inheritdoc/>
