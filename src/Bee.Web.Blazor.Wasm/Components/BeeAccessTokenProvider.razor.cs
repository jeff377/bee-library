...
{
    /// <summary>
    /// Holds the in-memory <c>AccessToken</c> and exposes it as a cascading
    ...
    /// State lives in the component instance only — on Blazor WebAssembly the
    /// token is lost on a full reload. Persistence across reload
    /// (sessionStorage via <c>IJSRuntime</c>) is deliberately out of scope for
    /// Phase 1d.
    /// </remarks>
