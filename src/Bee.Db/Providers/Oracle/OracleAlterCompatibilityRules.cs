...
{
    /// <summary>
    /// Rules for determining whether an Oracle column type change can be applied in place via
    /// <c>ALTER TABLE ... MODIFY</c>, or whether it requires a table rebuild. Counterpart to
    /// <see cref="MySql.MySqlAlterCompatibilityRules"/> and
    /// <see cref="PostgreSql.PgAlterCompatibilityRules"/>; Oracle 19c+ supports most
    /// in-place column changes within the same type family, so the rule shape mirrors the
    /// other family-aware providers (same-family → ALTER, cross-family → Rebuild).
    /// </summary>
    /// <remarks>
    /// Oracle is stricter than MySQL on numeric narrowing: <c>NUMBER</c> precision/scale
    /// reduction often requires the column to be empty (or a rebuild). The orchestrator
    /// gates narrowing changes through <c>UpgradeOptions.AllowColumnNarrowing</c>, so the
    /// rule here only classifies the structural family — narrowing is reported separately
    /// via <see cref="IsNarrowing"/>.
    /// </remarks>
    internal static class OracleAlterCompatibilityRules
    {
        /// <summary>
        ...
        /// Oracle's <c>CLOB</c> or unsized text storage.
        /// </summary>
