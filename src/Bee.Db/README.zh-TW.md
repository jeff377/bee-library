...
- `IFormCommandBuilder` -- 提供者專屬的 CRUD 命令產生（INSERT、UPDATE、DELETE、SELECT）
- `ICreateTableCommandBuilder` -- 提供者專屬的 DDL 產生
- `DbFunc` -- 資料庫感知工具方法（參數前綴、識別符引號、型別推斷）
- 內建提供者：SQL Server（同時支援 MySQL、SQLite、Oracle 的參數與引號語法）

### 結構描述探查

- `TableSchemaBuilder` -- 從即時資料庫探查資料表結構描述
- `TableSchemaComparer` -- 結構描述差異偵測，用於遷移情境
...
- **Provider Pattern** -- 資料庫專屬行為（引號、參數前綴、DDL）隔離於提供者介面之後，支援多資料庫擴充。
