...
- `Bee.Api.Core` / `Bee.Definition` (**breaking — wire**): 77 contract types (57 `Bee.Api.Core.Messages` request/response types + 15 `Bee.Definition` / `Bee.Api.Contracts` DTOs and non-`[Union]` collection items + the 5 `SerializableData*` DataSet/DataTable wire types) switch from integer `[Key(n)]` to `[MessagePackObject(keyAsPropertyName: true)]`; the MessagePack payload changes from a positional array to a property-name map, matching JSON. Deliberately excluded (kept on integer keys): `[Union]` polymorphic types (`FilterNode` / `FilterCondition` / `FilterGroup`) and the `ItemsForSerialization` proxy on `MessagePackKeyCollectionBase<T>`. [ADR-030](docs/adr/adr-030-messagepack-name-based-keys.md)
