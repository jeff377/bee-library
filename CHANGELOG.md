...
📄 Full notes and design context: [docs/changelogs/4.21.0.md](docs/changelogs/4.21.0.md)

### Security

- `Bee.Db`: the SQL Server "drop the column's default constraint" batch escapes the table identifier for the string literal it is embedded in. The statement runs through `EXEC('...')`, so bracket-quoting alone does not neutralise a single quote in the name — the two literals beside it in the same method already did this, and this one was missed. Reachable only through schema upgrade, which is a `LocalOnly` path.
...
- `Bee.Base`: `StringUtilities.IsEmpty(object?)` and `IsNotEmpty(object?)` are renamed to `IsEmptyText` / `IsNotEmptyText`. They stringify the value and then test the text, which is a different question from `ValueUtilities.IsEmpty(object)` — the two disagree for `Guid.Empty` and `DateTime.MinValue`, and the old names gave no hint which one a call site meant. The string overloads keep their names. One call site in the framework was affected.
- `Bee.Definition` / `Bee.Business`: `IBusinessObjectFactory.CreateBusinessObject` and the `CreateFormBO` / `CreateSystemBO` / `CreateLogBO` extensions no longer default `isLocalCall` to `true`. A local call bypasses `ApiAccessValidator` outright, so the two-argument form silently produced a business object with no access checks — the shape a host writing its own dispatcher would reach for first. Callers must now say which side of that boundary they are on. The business-object constructors keep their default for now; they are a more deliberate act than a factory call and changing them would touch every direct instantiation.
- `Bee.Api.Core`: `Messages.ApiErrorInfo` is removed (10 public API entries). `JsonRpcError` replaced it as the error shape on the wire; the type had no callers anywhere in the framework, and its only remaining sign of life was a test that constructed it and read the fields back — enough to show it as covered in the coverage report, not enough to show it was used. It was also registered as a wire contract despite being unreachable from the wire type closure, which is why the drift gate did not flag it.
...
- `Bee.Base` / `Bee.Api.Core` / `Bee.Business`: six further public surfaces with no consumer are removed. `Bee.Base.MemberPath` and `Bee.Base.ConnectionTestResult` had no reference anywhere and no documentation. `DataTableExtensions.NormalizeDateTimeMode(DataSet)` was a redundant overload; its `DataTable` sibling is still in use. `Bee.Api.Core.Registry.ApiContractRegistry` never had `Register<>` called, so its map was permanently empty while every payload paid for a `GetInterfaces()` loop — the job it advertised, converting a business object's plain result into an API type, is what `ApiOutputConverter` already does. `Bee.Business.AuditLog.ILogBusinessObject` and the `CreateLogBO` extension are removed, as are `ISystemBusinessObject.GetDefine` and `SaveDefine`: an axis interface exists so that one business object can call another, and neither querying the audit trail nor reading a definition is something a business object does to another one — definitions are reached through `IDefineAccess` directly. **The public methods stay on the concrete business objects, so what `JsonRpcExecutor` exposes over the wire is unchanged.**

### Added

- `Bee.Base`: `CompanyNotEnteredException` and `CompanyAccessDeniedException`. JSON-RPC `-32002` / `-32003` have been part of the published contract since [ADR-012](docs/adr/adr-012-session-company-context.md), but nothing ever threw them — `EnterCompany` threw `InvalidOperationException`, and `RepositoryDatabaseRouter` went as far as writing the error code's *name* into the message string. Both landed in the BCL allowlist `IsUserFacingException` reads and were mapped to `UserMessage` (`-32099`), so a user who opened a form without entering a company saw a dialog reading "CompanyNotEntered" and the front end had nothing to route on. Each now has its own exception type, its own `MapException` branch and its own reconstruction on the client.
...
- `Bee.Api.Client`: company-level number formatting takes effect again. `NumberFormatApplier` stopped running when an earlier release removed the server-side bake and noted that the consuming side would take it over — the consuming side never did, so a company's number formats had no effect on any head. `FormDefinitionLoader.GetLocalizedSchemaAsync` now applies it on both of its paths, including the early return for an empty language, with the company supplied through the new `CompanyAccessor` init property. It takes a delegate rather than a value because the entered company changes over a session's life. The Avalonia and Blazor heads both benefit.
...
- `Bee.Db` / `Bee.Api.Client` / `Bee.Api.Core`: five library `await`s now use `ConfigureAwait(false)`, matching the convention the surrounding code already followed. They sat on the path a UI host reaches through `JsonRpcExecutor.Execute`, which blocks on the asynchronous core — resuming on a captured `SynchronizationContext` there is the classic deadlock. `Execute` now documents that it blocks and points asynchronous callers at `ExecuteAsync`.
- `Bee.UI.Core` / `Bee.ObjectCaching` / `Bee.Base`: three `catch` blocks that swallowed every exception now catch only the failures they were written for. The client's endpoint probe treated any fault as "server unreachable" and sent the user to the connection-setup view; the cache's file-watch token treated any fault as "no known write time"; the `DataTable` JSON converter treated any fault as "leave the value as-is". Unexpected faults now surface instead of being reported as something else.
- `Bee.ObjectCaching`: concurrent misses on the same cache key now produce one object instead of one per caller. `ObjectCache<T>.Get` and `KeyObjectCache<T>.Get` were read-create-write over a provider that offers no atomic get-or-create, so a cold key reached by several requests was built several times — each deserialising the same definition file or issuing the same query. Duplicate instances were the worse half: `SessionInfo` is cached this way, so `EnterCompany` performed through one instance was invisible to the request holding the other, and the resolvers that detect a reload by reference comparison read a duplicate as a change that never happened. De-duplication is per key and holds only while a creation is in flight, so nothing accumulates for caches keyed by an access token.
...
- `Bee.Definition` / `Bee.Api.Client`: two places that handed out or rebuilt process-wide cached definition state under concurrency are fixed. `FormTable.RelationFieldReferences` built its reverse index behind an unguarded null check, so two requests touching the same cached schema could build it twice and receive different instances — and could surface the builder's validation exception from a plain property getter. `FormDefinitionLoader.GetLocalizedSchemaAsync` returned the shared cached schema when no language was given, despite documenting that the cached instance is never handed out; `CultureInfo.InvariantCulture.Name` is the empty string, so that was the common path.
- `Bee.Db` / `Bee.ObjectCaching`: the `DatabaseSettingsChanged` event now fires on a reload rather than on every load. Its only publisher was the settings cache's load path, and `DbConnectionManagerService` clears its connection cache in response — reached from inside its own `GetOrAdd` value factory, so every first settings miss discarded every connection entry built so far. A genuine reload (the settings file is watched) still fires, which is what carries an external edit through to the connection cache. `DbConnectionManagerService` also implements `IDisposable` and unsubscribes; a static event holds its subscribers for the life of the process.
- `Bee.Business`: `EnterCompany` resolves the company's roles, capabilities and record-scope identity before writing any of them to the session. The session object is shared by every concurrent request on the same access token, and `CompanyId` and `Roles` are the two inputs authorization reads — interleaving the writes with the lookups left a window spanning three database-capable calls in which a parallel request could authorize against the new company with the previous company's roles.
- `Bee.Definition`: permission lookups build their indexes once at construction instead of scanning every grant on each check. `CompanyRolePermissions` is an immutable snapshot held in a process-wide cache, and its grant count is roles × models — thousands to tens of thousands in an ERP. Lookups now cost O(roles the user holds) rather than O(grants in the company): measured at 200 roles × 100 models, a check went from 23.4 µs to 0.08 µs, and no longer degrades as a deployment grows.
- `Bee.ObjectCaching`: `ApiKeyCache` caches a miss for one minute instead of five. Its key is whatever identifier the caller put in the `X-Api-Key` header and the gate runs on every request, so every negative entry is attacker-chosen — and the cache has no size limit. Caching misses still helps a misconfigured client retrying one revoked key, which is why it is kept; it never helped against probing of *random* identifiers, since each distinct one reaches the database once regardless.
