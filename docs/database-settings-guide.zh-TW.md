...
|------------|---------|---------|------------------|
| `st_user`、`st_session`、`st_company`、`st_user_company` | common | 任何 | 固定為字串 `"common"`（框架契約：`DatabaseItem.Id == "common"`） |
| `st_employee`、`st_department` | company | 單一公司 | `company` 分類那筆的 `Id` |
| `st_employee`、`st_department` | company | 多租戶 | 依當前租戶選 `companyXXX` 對應的 `Id`（如 `$"company{tenantId:D3}"`） |
  ...
  <Categories>
    <DbCategory Id="common" DisplayName="共用資料庫">
      <Tables>
        ...
        <TableItem TableName="st_company" DisplayName="公司" />
        <TableItem TableName="st_user_company" DisplayName="用戶可進入公司" />
      </Tables>
    </DbCategory>
    <DbCategory Id="company" DisplayName="公司資料庫">
      <Tables>
        <TableItem TableName="st_department" DisplayName="部門" />
        <TableItem TableName="st_employee" DisplayName="員工" />
