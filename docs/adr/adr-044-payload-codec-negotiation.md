...
[ADR-014](adr-014-jsonrpc-plain-public-default.md) 已經為純 JavaScript 前端鋪過一條路：
以 HTTPS 為信任界線、走 `PayloadFormat.Plain`，並為此把七個 BO 方法從 `Encrypted`
降級為 `Public`。`samples/Web.Js.Demo` 就是那條路的示範——零建置、零 npm、純 `fetch`。

**那個決策仍然有效，本決策不取代它。** Plain 仍是 JS 前端的預設路徑，那七個方法仍是
`Public`，既有的示範也不需要改動。

本決策要處理的是 ADR-014 明確劃在範圍外的那一半：**當部署確實需要應用層加密時，
JS 前端該怎麼辦。** ADR-014 在〈為何不為 JS 前端做「JS 版加密管線」〉一節評估並拒絕了
這件事，三條理由值得逐條對照，因為其中兩條的前提已經改變：

| ADR-014 的理由 | 現在的狀況 |
|---|---|
| 「JS 端與 .NET 端的加密管線要逐 byte 對齊，測試成本高」 | 這正是 `wire-fixtures/` 在解的問題：黃金樣本由 .NET 產生並驗證，跨語言雙向對照，對齊不再靠人工比對 |
| 「JS 版加密管線會在多個前端框架各自再實作一份」 | 這正是把它做成**一個 npm 套件**而不是讓每個應用自己寫的理由；框架各自實作是「沒有套件」的後果，不是「有加密」的後果 |
| 「降級 ProtectionLevel 的動機就是讓 JS 不用碰加密；若 JS 仍要實作，等於白做」 | **這一條是真正的立場分歧**，而它不是技術問題 |

第三條的分歧在於「HTTPS 夠不夠」。ADR-014 的答案是夠——對多數部署也確實夠。
但那個答案把「需要應用層加密」的部署（TLS 終止後有中介存取、或合規要求 payload 加密）
擋在 JS 之外，只能改用 Blazor WASM 或放棄 JS 前端。

**本決策讓這件事回到部署方手上**：不需要應用層加密就繼續走 Plain，ADR-014 的路徑完全不變；
需要就宣告 `codec: json` 走 Encrypted。兩者共用同一組 API 與同一個後端，差別只在信封裡的
一個欄位。

值得一提的是，ADR-014 提到的另一個選項「不如改用 Blazor WASM」在本決策之後仍然成立，
且各有取捨：Blazor WASM 免除跨語言的一切，代價是執行環境與前端生態。
