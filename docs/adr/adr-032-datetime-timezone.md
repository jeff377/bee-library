...
**`Now()` 回「所在側空間的當下」**——伺服端 UTC、用戶端使用者時區。這是一個定義而非兩種語意：
「當下這一刻，以周遭儲存格所在的空間表示」。與 `Today()` 的不對稱直接來自 D4：`Date` 欄位
不轉換故兩側必須同值，`DateTime` 欄位雙向轉換故各在自己的空間。

> 不採「一律回 UTC」：用戶端把 UTC 填進使用者時區空間的儲存格後，Connector 送出時會再轉一次，
> 而它無從得知某儲存格是運算式填的——這條路徑罕見且靜默，是最難發現的組合。其邊際成本近乎零：
> 伺服端本來就需要使用者時區（`Today()` 兩側都要算使用者時區的今天），接縫無論如何都得是
> 時區感知的。
>
> 不拆成 `Now()` / `UtcNow()` 兩個函式：那把「這段運算式跑在哪一側」丟給定義檔作者，
