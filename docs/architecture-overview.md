...
1. [架構核心理念](#1-架構核心理念)
2. [架構模式定位](#2-架構模式定位)
3. [FormDefine 定義中樞](#3-formdefine-定義中樞)
4. [FormLayout 介面層定義](#4-formlayout-介面層定義)
5. [DbTable 資料層定義](#5-dbtable-資料層定義)
...
BeeNET 採用**定義導向架構（Definition-Driven Architecture）**，以 `FormDefine` 作為系統的唯一定義來源（Single Source of Truth），統一驅動 UI、資料庫結構與業務邏輯，解決傳統 ERP 開發中規格分散三層、重複實作、難以維護的核心痛點。
...
| **Clean Architecture** | 依賴方向向內、Domain Core 最穩定、Use Case 隔離 | FormDefine 為 Domain Core；BO 為 Use Case；Repository 為 Interface Adapter |
| **MVVM** | ViewModel 隔離 View 與 Model、雙向綁定 | FormDefine 驅動 ViewModel 結構；DataSet 為 Model |
...
`FormDefine` 是 BeeNET 架構的核心，一份**跨層共用的結構描述模型**。

### 職責範圍

- **欄位定義**：欄位名稱、資料型別、長度、預設值
- **行為定義**：必填、唯讀、隱藏、驗證規則
- **關聯定義**：與其他表單（FormDefine）之間的主/明細關係
- **SQL 產生依據**：Repository CRUD 從 FormDefine 動態產生 SQL
- **UI 推導來源**：FormLayout 從 FormDefine 推導版面結構
- **DB 推導來源**：DbTable 從 FormDefine 推導資料表結構
...
graph TD
    subgraph 產生 FormDefine
        A1["AI 智能生成（自然語言 → FormDefine）"]
        A2["視覺化工具調整（拖拉欄位與關聯）"]
    end

    A1 --> FD["FormDefine（表單定義）"]
    A2 --> FD
...
FormLayout 與 DbTable 預設由 FormDefine 推導產生，但支援獨立調整：
...
重新推導「預設值」
    ↓
與現有 FormLayout / DbTable 做 diff
    ├─ 未手動調整的部分 → 更新
    └─ 已手動覆寫的部分 → 保留
```

這確保 FormDefine 演進時，人工調整的客製設定不會被覆蓋。

---

## 4. FormLayout 介面層定義

`FormLayout` 是 FormDefine 在 UI 維度的投影，描述表單的視覺配置。
...
`DbTable` 是 FormDefine 在資料庫維度的投影，負責描述並維護資料表結構。

### 職責

- 從 FormDefine 推導資料表欄位、型別、長度
...
```
FormDefine：欄位 Amount，型別 Decimal
    ↓ 推導
DbTable 預設：DECIMAL(18, 2)
    ↓ DBA 調整（獨立於 FormDefine）
DbTable 實際：DECIMAL(24, 6)  +  INDEX  +  DEFAULT 0
```

FormDefine 不需要知道資料庫層的最佳化細節，DbTable 可獨立演進。
...
{
    // CRUD：透過 FormDefine 驅動的 Repository
    public void Save(DataSet ds)
    {
        // 1. 依 FormDefine 驗證 DataSet 資料
        // 2. 呼叫 FormDefine-driven Repository 執行 INSERT / UPDATE
    }
...
| 軌道 | 適用作業 | SQL 來源 | 特性 |
|------|----------|----------|------|
| **FormDefine 驅動** | CRUD（新增、修改、刪除） | FormDefine 動態產生 | 定義一處，自動同步；無需手寫 SQL |
...
這 80% 的作業量讓 FormDefine 驅動，開發者只需定義，不需撰寫程式。

報表與批次作業的 SQL 往往是多表 JOIN + GROUP BY + 動態條件，或需要控制交易邊界與分批策略，強行套入 FormDefine 反而增加不必要的複雜度。
...
BeeNET 以 MVVM 模式整合前端，FormDefine 直接驅動 ViewModel 的 binding 結構。
...
| **ViewModel** | 由 FormDefine 推導 | 欄位行為、驗證規則、binding 結構 |
...
使用者操作
    ↓↑（雙向綁定）
ViewModel（FromDefine 推導 binding 結構）
    ↓↑
DataSet（Model）
    ↓
BO.Save(DataSet)
...
FormDefine 改變時，ViewModel 的 binding 結構自動更新，View 無需手動調整。
...
| 模式 | 自由度 | 實作方式 | 適用情境 |
|------|--------|----------|----------|
| **NoCode** | 中 | FormDefine → FormLayout + DbTable 自動產生 | 標準流程、資料導向表單 |
...
每一次 AnyCode 客製所發現的通用模式，可沉澱回 FormDefine 或 BO 基類，使下一次開發更自動化。
...
│  ├─ FormDefine-driven（CRUD SQL 自動產生）           │
