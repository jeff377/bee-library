...
| [Analyzer 規則](analyzer-rules.zh-TW.md) | 隨套件提供的建置期診斷：規則清單、如何調整嚴重度、版本政策 |
...
| [時間型別總覽：Date、DateTime、Time](temporal-types.zh-TW.md) | 跨層對照參考：三者如何選擇、各自在資料庫、`DataSet`、程式碼與三種序列化中的承載方式，以及自寫 SQL 時如何宣告日曆日欄位 |
...
- **[`adr/`](adr/README.md)** — 架構決策紀錄，記錄重大設計決策的背景與理由
