...
3. **業務案例 = Northwind 核心 8 表**，但鍵結構用 bee 慣例（見「Northwind → bee 模型對應」表）
4. **lookup 機制已完整落地於 main**（見 `docs/archive/plan-avalonia-lookup-edit.md`）：`DisplayFields` 複合顯示、`GetLookup` API、`LookupDialog`、主表 + 明細 InCell lookup 全可用（桌面）
5. **現成範例**：`samples/Define/Project.FormSchema.xml`（lookup 定義）、`samples/Avalonia.Demo/`（前端接線）、`samples/Bee.Samples.Shared/DemoSchemaSeeder.cs`（建表 + 種子）
6. **相關 skill**：`bee-sample-add`（建專案，注意目標放 apps/、自建 slnx、UI+head 結構）、`bee-scaffold-from-formschema`（從 FormSchema 產 sidecar）、`bee-add-bo-method`（OrderBO 若需新 action）、`demo-smoke`（端到端冒煙）
7. **協作慣例**：本機可 build/test → 直接改 main；UI 編譯過即交付使用者自測；每階段 commit 前 `./test.sh` 全綠（改了 src 底層更要全綠）
