...
| 3 | 框架系統表 + 擴充：Department（沿用框架）+ Employee（框架 st_employee + Northwind 欄位擴充，dept→Department lookup） | 📝 待做 |
...
- **框架系統表（`st_`，沿用框架 `Defaults`）**：`Employee`（`st_employee`）、`Department`（`st_department`）。框架 `src/Bee.Definition/Defaults/` 已散佈標準定義，demo **不重新發明** —— 沿用框架定義，**保留框架所需欄位**，需要時在其上**擴充**業務欄位。
...
| 部門（Northwind 無，框架有） | **框架 `st_department`**：沿用 Defaults，作為 Employee 的 dept lookup 來源 |

> **擴充框架定義的機制**：demo 的 `Define` 目錄放一份 `Employee.FormSchema` + `st_employee.TableSchema`（框架 6 欄 + Northwind 擴充欄），覆蓋框架 `Defaults` 的同 ProgId。執行時須確認 `IDefineAccess` 的載入優先序（demo Define 優先於框架 embedded Defaults）—— 見 `framework-defines-distribution`（已完成、已封存）。Department 若不需擴充則直接沿用 Defaults。
...
- ✅ **框架系統表現成**：`src/Bee.Definition/Defaults/` 已有 `Employee` / `Department` 標準定義（含 Employee→Department lookup + 部門經理多欄 mapping），demo 沿用 + 擴充，不重新發明
...
| 專案 | 角色 |
|------|------|
| `Bee.Northwind.Define` | FormSchema / FormLayout / TableSchema（含擴充版 Employee；前端無關） |
...
| ProgId | 表（前綴） | 來源 | 展示重點 |
|--------|-----------|------|---------|
| `Department` | `st_department` | 框架 Defaults 沿用 | 純主檔；Employee 的 dept 來源 |
| `Employee` | `st_employee` | 框架 Defaults + Northwind 擴充 | 保留框架 6 欄；擴充 `title` / `hire_date`；dept→Department lookup（部門經理帶主管） |
...
└── Bee.Northwind/                      # 開發期借住 bee-library；畢業移至 repo bee-northwind-avalonia
    ...
    ├── Bee.Northwind.Define/           # FormSchema / FormLayout / TableSchema XML
    │   ├── FormSchema/                 #   Employee(擴充), Category, Supplier, Customer, Shipper, Product, Order
    │   │                               #   （Department 視需要；不擴充則沿用框架 Defaults）
...
- Server 載入框架 `Defaults`（自帶 `st_employee` / `st_department` / `st_user` 等系統表）+ demo `Define`（業務表）；seeder 首啟建表（`st_` + `ft_`）+ 種子
- `Bee.Northwind.Define` 先放一張最小業務主檔（`Category`），Desktop 登入（demo/demo）→ CRUD 跑通
...
- `Department`：沿用框架 `Defaults`（不需擴充則 demo Define 不放、直接用框架定義）
- `Employee`：demo Define 放擴充版 —— **保留框架 6 欄**（`sys_*` / `dept_rowid`→Department / `user_rowid`）+ **擴充** Northwind 純資料欄（`title` 職稱 / `hire_date` 到職日）；沿用框架的 `dept_rowid → Department` lookup（部門經理帶主管，多欄 mapping）
- 仍**零 C# 業務碼**
...
| 風險 | 緩解 |
|------|------|
| Northwind schema 與 bee 模型落差（int FK / 複合鍵 / 表前綴） | 採 bee 慣例（見「Northwind → bee 模型對應」表）；框架表 st_ 沿用 Defaults、業務表 ft_ 自定義 |
| 擴充框架 Employee 定義時覆蓋 / 載入優先序不明 | 確認 `IDefineAccess` 載入序（demo Define 優先於框架 embedded Defaults）；對齊 `framework-defines-distribution` |
...
- （觀察）擴充框架系統表（在 `st_employee` 加業務欄）的定義覆蓋/合併機制是否順手，是否需框架層支援「定義增量擴充」而非整份覆蓋
...
4. **框架表 vs 業務表分層**：`Employee`(`st_employee`) / `Department`(`st_department`) 是**框架系統表**，沿用 `src/Bee.Definition/Defaults/` 定義；Employee 在 demo Define 擴充 `title` / `hire_date`（保留框架 6 欄）。主管 = 框架 dept→部門經理（**不做員工自關連**）。業務表 `ft_`（Category/Supplier/Customer/Shipper/Product/Order/OrderDetails）由 demo 自定義
