...
| 2 | iOS 免實機 AOT build 驗證；執行期半 B round-trip 延後待實機 | 🚧 免實機 AOT build ✅；iOS 實機執行期 round-trip 待實機+簽章 |
...
- [ ] **需實機（延後，且需簽章）**：取得 iOS 實機 + 簽章憑證後，Release（AOT）跑一次完整序列化 round-trip（載入定義 → 開表單 → master-detail）
  - 通過 → XML 這條收工
...
- **殘餘風險很低**：半 A 已由 descriptor 覆蓋（Android 實跑 + iOS link 大小雙證）、AOT 編譯期 0 錯誤、半 B 反射 fallback 已於 `ios-xmlserializer-ambiguous-add` 修好。**唯一未驗**＝iOS 實機**執行期** round-trip（需實機 + 簽章）。
