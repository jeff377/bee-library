...
| 欄位長度 | `DbType="String" Length="10"`——上限 10 碼，值為數字＋英文，未滿 10 碼亦可 |
...
### 公司代號格式：10 碼字串上限，數字＋英文

`sys_company_id` 一律宣告為 `DbType="String" Length="10"`。**10 是欄位容量上限，不是固定長度**
——實際值不超過即可，`C001` 這種 4 碼的值完全合法。字元集為數字與英文字母。

長度有上限且短，對本 plan 是實質好處：它是每張業務表都會多出來的欄位，也會進到每個複合
唯一索引的第一段，寬度可預期，索引成本才壓得住。

`FieldDbType` 只有變動長度的 `String` / `Text`，沒有固定長度型別；欄位負責上限，
字元集則由**驗證層**把關。
...
1. **`st_company.sys_id` 目前是 `Length="20"`**（[st_company.TableSchema.xml](../../src/Bee.Definition/Defaults/TableSchema/common/st_company.TableSchema.xml)）。
   ...
   建議**收窄為 10** 讓容量上限留在單一事實來源；既有部署需在升級前檢查有無超過 10 碼的公司代號。
