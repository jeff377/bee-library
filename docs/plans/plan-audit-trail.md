...
| 1 | 登入記錄 | ① | `st_log_login` | *(定案後建立)* | 📝 待做 |
| 2 | 異動記錄（含安全⑥） | ③(+⑥) | `st_log_change`（DiffGram `changes_xml` 單表）+ outbox；EAV `st_log_change_field` 選配 | *(定案後建立)* | 📝 待做 |
| 3 | 執行記錄 | ④ | `st_log_exec` | *(定案後建立)* | 📝 待做 |
| 4 | 檢視記錄 | ② | `st_log_access` | *(定案後建立)* | 📝 待做 |
| 5 | 系統/錯誤（選配） | ⑤ | `st_log_trace` | *(定案後建立)* | 📝 待做 |

- 六軸→收斂成「基礎設施 + 5 項」（軸⑥安全按 D3 併入項 2、以旗標區分）。
- 排程：**0 → 1 登入記錄 → 2 異動記錄 → 3 執行記錄 →** 4 檢視 →（5 系統選配）。每項流程固定：**出細部 plan → 逐點定案 → 執行**。
...
| `user_rowid` | Guid, null | 觸發者（對應 common `st_user`；登入失敗可為 null） |
| `user_id` | String | 觸發者帳號（即使 `user_rowid` 未知也留字串，對齊 SAP `USERNAME`） |
| `company_id` | String, null | 租戶情境（登入前事件為 null；log 集中一庫時用此區分公司） |
...
> 沿用框架 `sys_rowid` / `sys_no` 主鍵慣例（[`src/Bee.Definition/SysFields.cs`](../../src/Bee.Definition/SysFields.cs)）；日誌專屬欄用 `log_` / 語意命名，不硬套 `sys_insert_time`（事件時刻語意不同）。

### 4.3 各表 schema

**① `st_log_login`（登入記錄）** — 軸①
- 共通欄 +
- `event`（Integer/enum）：`Login` / `Logout` / `LoginFailed` / `LockedOut`
- `result`（Integer）：Ok / Fail
- `fail_reason`（String, null）：密碼錯誤 / 帳號鎖定 / 已停用 …（**不含明文密碼**，見 security.md）
...
**④ `st_log_access`（檢視記錄，opt-in）** — 軸②
- 共通欄 + `prog_id` + `table_name` + `row_key` + `field_name`（被檢視的敏感欄）+ `purpose`（String, null：合規理由 / log domain）
- **預設關閉**；只對「標記為敏感」的欄位記錄（見 §6.2）。
...
- **依年分片/分區**：`log` 分類支援一 CategoryId 對多 DatabaseItem（`log_2025` / `log_2026`，見 [database-settings-guide.md](../database-settings-guide.md) 情境 4），或 DB 原生 partition by `log_time`；清理以 drop partition 為單位（遠快於 delete）。
...
- **D2 異動 log 交易邊界 → transactional outbox**：業務交易內寫 outbox（強一致、同庫免跨庫交易）→ 背景 flush 到 log DB（見 §7.1）。
- **D3 安全/組態（軸⑥）→ 併入 `st_log_change`**：以 `is_sensitive` 旗標 / `prog_id` 過濾區分，不另建表（§4.3）。
- **D4 首階段範圍 → 本輪只出 plan，暫不實作**。日後啟動時，建議由 **Phase 0+1（基礎設施 + 執行 + 登入）**入手（單一收斂點、無欄位級 diff 複雜度、最快見效）。
