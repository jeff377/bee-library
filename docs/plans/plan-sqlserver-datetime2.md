...
故必須把 `DbTypeMapper.Infer` 的 `DateTime → DbType.DateTime` 改為 `DbType.DateTime2`。
此為**跨 provider 共用檔**（SQL Server / PostgreSQL / MySQL / SQLite / Oracle 皆走此路），
由各 provider 的 `[DbFact]` round-trip 測試驗證相容性。
...
## 階段 3：寫入參數型別 `DbType.DateTime2`（跨 provider）

[DbTypeMapper.cs:28](../../src/Bee.Db/DbTypeMapper.cs#L28)：
```csharp
if (type == typeof(DateTime)) return DbType.DateTime2;   // was: DbType.DateTime
```
影響 SQL Server / PostgreSQL / MySQL / SQLite / Oracle 共用寫入路徑；由各 provider round-trip 測試驗證。
...
- **跨 provider 共用 `DbTypeMapper`**（寫入參數推斷）：階段 3，影響全部 5 個 provider 的 DateTime 寫入。
- MySQL / PostgreSQL / SQLite / Oracle 的 **DDL 型別對映不動**（維持各自 datetime(6)/timestamp 等）；
  僅寫入參數型別由 `DbType.DateTime` 改 `DbType.DateTime2`，語意等價、精度更完整。
