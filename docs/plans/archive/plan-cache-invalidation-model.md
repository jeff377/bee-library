...
框架體檢（`plan-framework-review.md`，未入版控，見 `docs/internal/`）指出 `Bee.ObjectCaching` 有 8 個定義快取以 `if (_storage is FileDefineStorage)` 做執行期具象型別判斷，屬能力洩漏。追查後發現真正的問題更深一層：**`CacheItemPolicy` 只表達了檔案相依，DB 相依完全在快取類之外**，兩套失效機制性質不同、且靠字串約定隱性對齊。
