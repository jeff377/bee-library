...
**狀態：✅ 已完成（2026-07-24）— 子項 0–4 寫入側 + 讀取側（log-query）全數完成並封存；`framework-reserved-names.md` §1.3／§2.1（英＋繁中）已列齊 5 張 `st_log_*` 表與 `AuditLog` progId。唯一未做的 per-form 稽核規則已明確歸為另案（未來增強，預設全記相容）**
