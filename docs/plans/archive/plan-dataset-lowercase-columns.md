...
**狀態：✅ 已完成（2026-07-24）— 程式遷移完成；CHANGELOG breaking 條目已於 4.14.0 發佈（ADR-029）；多 DB provider 容器全回歸於 2026-07-24 通過（SQL Server / PostgreSQL / MySQL / Oracle 四庫，0 provider skip）**
...
| 4 | 持久化相容 + 全 repo 回歸 | ✅ 已完成（2026-07-24）— 四庫容器全回歸通過：Bee.Db 1411 / Bee.Repository 141 / Bee.Business 279，SQL Server/PG/MySQL/Oracle 皆實際執行（0 provider skip）；audit 歷史以「讀取端大小寫無關」相容；CHANGELOG breaking 已於 4.14.0 標記 |
