...
**狀態：✅ 已完成（2026-07-24 結案）— 半 A+B 免實機驗證通過；iOS 實機 AOT 發佈已由其他專案的實機發佈佐證，本 plan 的實機形式收尾不再需要。條件式階段 3／4 未觸發（另案，需要時再啟）**
...
| 2 | iOS 免實機 AOT build + 半 B（reflection-only）驗證 | ✅ 已完成（2026-06-27，免實機；實機 AOT 發佈已由其他專案實機發佈佐證，形式收尾不再需要） |
| 3 | （條件）實機 fallback 不過 → 改走 Sgen 預編序列化組件 | ⏸️ 未觸發（實機 AOT 已佐證可行，無需 Sgen；需要時再啟） |
| 4 | （條件）行動端若改用 MessagePack wire 上線 → 補 iOS Release 驗證 | ⏸️ 未觸發（另案，待行動端實際改用 MessagePack wire 時再啟） |
