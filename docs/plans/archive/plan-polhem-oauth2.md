...
| 6b | 首發前修正：依健檢結果修掉全部發現，公開 API 定型 | ✅ 已完成（2026-09-14） |
| 7 | 首發 `Polhem.OAuth2.*` 1.0.0 | ✅ 已完成（2026-09-15） |
| 8 | 凍結舊套件與舊 repo | ✅ 已完成（2026-09-15）；舊的 `NUGET_API_KEY` 仍供其他 `Bee.*` 套件使用，不撤銷 |
| 9 | 回寫 bee-library：future-work 與演練結果 | ✅ 已完成（2026-09-15） |
...
1. **（使用者操作，以 jeff377 身分於 nuget.org）** 五個 `Bee.OAuth2.*` 標 deprecated，原因選 Legacy，並填替代套件：
   ...
   **完成（2026-09-15）**：使用者改為要求 agent 在已登入 jeff377 的 Chrome 操作。
   ...
   - NuGet API 在存檔後約 5 分鐘內反映了全部五個套件：所有版本都標為 Legacy，替代套件與上表一致。
...
4. 舊 repo 的 `NUGET_API_KEY` 若只給這個 repo 用，到 nuget.org 撤銷（使用者操作）。
   **不撤銷（2026-09-15）**：使用者確認這把 key 仍供其他 Bee.* 套件發佈使用，前提「只給這個 repo 用」不成立。agent 原本依各 repo 的 secret 設定日期不同，推測它是舊 repo 專用，這個推測是錯的：secret 的設定日期看不出 key 是否相同。
...
**完成（2026-09-15）**：
- `docs/repo-ops/future-work.md` 的「已占名」表：
  - 補上 repo 與三個套件。
  - org profile 一列改為「已列出 Polhem.OAuth2」。
  - 前綴保留一列註明前提已成立（仍未申請，申請列在範圍外）。
  - 商標檢索一列改為「發佈前已重查」，初查表後補上重查結果與智慧財產局的新網址。
- 新增「bee-oauth2 演練結果」一節：
  - 對照「要一併決定的」五項。
  - 凍結順序與 Trusted Publishing。
  - 踩到的雷：首發前健檢的時機、每種 client 類型都要實測、nupkg 內容、org profile 連結、secret 無法辨識 key、samples 設定檔、IDE 改寫 `.slnx`。
  - 只寫做法與教訓，不寫健檢發現的弱點細節。
- 索引狀態改為已完成。依規範，由使用者要求時才封存。
