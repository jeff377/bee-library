...
> XML 路徑唯一的實際缺口（控制字元與 NUL 讓序列化擲例外）已於 `7d867913` 另案修正。
> 決策已記入 ADR-040 第九節。本 plan 保留作為評估與實測紀錄，以下「設計」「待決事項」「影響的檔案」皆未執行。
>
...
> **補記（2026-09-11，`7d867913` 之後）**：XML 寫入端補上不允許字元的處理後，以真實的
> `AuditDiffGram.Serialize` 與 `ChangeDiffGramReader.Read` 重測：CR、CRLF、控制字元 U+0001、NUL、U+FFFE
> 不論當新值或舊值皆讀回原值，落單 surrogate 換成 U+FFFD（與 JSON 相同）。上表 XML 在這幾列的
> 「靜默改成 LF」「序列化擲例外」是修正前的行為。一般資料的 payload 與修正前逐字相同（校準），
> 體積與還原度數字不受影響；效能數字未重測。
