...
**狀態：✅ 已完成（2026-07-03；尾巴於 2026-07-24 確認了結）— Bee.UI.Avalonia 實作落地；分類 gate 全公司一致。DB 綁定測試（EnterCompanyJsonRpc / SystemBusinessObjectEnterCompany）已隨後續多次 release 於 CI 驗證通過**
...
| Grid 內敏感欄位 | `FormField.SensitiveCategory` | `GridControl.EnumerateVisibleColumns`（[GridControl.cs:1135](../../../src/Bee.UI.Avalonia/Controls/GridControl.cs)） | 同欄位規則（Grid 動作本身不參與，由表單模式驅動） |
...
3. **降級套用不動快取定義**（守 definition-immutability；client layout 為每視圖新生成，可就地 mutate）：`有效可見 = layout.Visible && cap.Visible`、`有效唯讀 = layout.ReadOnly || cap.ReadOnly`、按鈕 `IsVisible = cap.Can(action)`。detail Grid 動作不參與（由表單模式驅動）。
...
| Grid 內敏感欄位降級點 | `GridControl.EnumerateVisibleColumns`（篩 Visible） | ✅ 已就緒（Grid 動作由表單模式管，不在此列） |
...
| **detail Grid 動作不參與 capability** | **由表單模式驅動，非權限** | detail Grid 與 master 同屬一個 aggregate，明細列能否編輯跟隨表單編輯模式（`AllowEdit`）；「能否進入編輯」已被主 model 的 Create/Update 在工具列命令擋掉。故**不做 Grid 動作降級**（無 `ResolveGridActions`）。惟 Grid 內**敏感欄位（column）仍降級**——那是資料分類，與 aggregate 編輯權正交。 |
...
- 新增 `IElementCapabilityResolver` + 實作於 `Bee.UI.Core`（純函式，對齊後端 `ScopeResolver` 風格）：
  ...
  - `ResolveField(FormSchema schema, string fieldName, tableName, snapshot)` → `FieldCapability { Visible, ReadOnly }`（`SensitiveCategory == None` → 全放行；含 Grid 內欄位）
...
- **Grid 內敏感欄位**：比照欄位規則，`EnumerateVisibleColumns`（1135 行）吃到降級後的 column。**Grid 動作（Add/Edit/Delete）不做 capability 降級**——detail 與 master 同 aggregate，由表單編輯模式（`AllowEdit`）驅動，權限已在工具列命令擋掉。
...
- Avalonia glue 測試（按鈕 `IsVisible`、欄位 Visible AND / ReadOnly OR）。
...
IElementCapabilityResolver
  ...
  // 註：detail Grid 動作（Add/Edit/Delete）不做 capability 降級——
  //     同 aggregate、由表單編輯模式驅動，權限已在工具列命令擋掉。
```
