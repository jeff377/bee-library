...
> 已移交的 D-3 / D-5 仍由 [plan-property-grid-control.md](../plan-property-grid-control.md) / [plan-tree-view-builder.md](../plan-tree-view-builder.md) 承接，不受本次封存影響。
...
> 以上為**修正前**的評分。修正後的重評見〈修正後評分〉一節
> （九面向 8.60／八面向 8.61／十一面向 8.36），
> 但那是只針對既有項目的增量重評，不是重跑體檢。
...
| 🔀 已裁決、部分完成或移交 | 4 | D-2（文件面已修，程式面裁決不接載入期）、D-4（裁決保留）、D-3 / D-5（移交 [plan-property-grid-control.md](../plan-property-grid-control.md) / [plan-tree-view-builder.md](../plan-tree-view-builder.md)） |
...
## 修正後評分（2026-08-07，v4.18.0 發版後）

| # | 面向 | 上輪 | 本輪修前 | **修正後** | 變化 | 依據 |
|---|------|------|---------|-----------|------|------|
| 1 | 架構分層 | 8.8 | 8.6 | **8.6** | — | A-1 / A-3 未動 |
| 2 | 相依分層 | 9.2 | 9.0 | **9.0** | — | A-2 / A-4 未動 |
| 3 | 安全性 | 7.8 | 7.0 | **8.6** | ▲1.6 | S-1/S-2/S-3 全修 + BEE3003 gate + 明文密碼；S-4 連兩輪未修 |
| 4 | 維護性 | 8.5 | 8.5 | **8.5** | — | M-1 / M-2 未動（中文 #region 27→31） |
| 5 | 散落／不必要類別 | 7.5 | 7.0 | **7.5** | ▲0.5 | D-1 駁回＝扣掉誤報，非改善 |
| 6 | 序列化一致性 | 7.0 | 8.5 | **9.2** | ▲0.7 | Z-1 / Z-2 全修；Z-3 landmine 仍在 |
| 7 | 公開 API 表面 | 8.5 | 8.5 | **9.0** | ▲0.5 | X-1/X-2/X-3 全修；M-1 撞名仍在 |
| 8 | 測試品質與覆蓋 | 8.2 | 8.5 | **8.5** | — | N-6 發現並修，但推翻『並行序列化已處理』的前提；T-1/T-2 未動 |
| 9 | 文件漂移 | 4.5 | 6.0 | **8.5** | ▲2.5 | C-1~C-7 / X-2 / X-3 / Z-2 全修，五項檢查全清；無自動化 gate |
| 10 | 效能／熱路徑 | — | 6.0 | **6.8** | ▲0.8 | 僅 P-1；P-2/P-3/P-4 未實測 |
| 11 | 並行與全域狀態 | — | 7.0 | **7.8** | ▲0.8 | N-3/N-4/N-6 修；N-2/N-5 遂延，兩者影響 production 行為 |

- **九面向平均：8.60**（修前 7.96，▲0.64）
- **八面向平均（不含文件）：8.61**（修前 8.20，▲0.41）
- **十一面向平均：8.36**（修前 7.69，▲0.67）

### ⚠️ 這組分數的效力範圍

**這不是重跑體檢。** 原始分數來自 10 個平行代理對十一面向的全量掃描；這組是**只針對
plan 內既有項目**的增量重評——沒有重新全量掃描，因此**抓不到本輪之後新產生的問題**，
也抓不到原掃描本來就漏的面向（C-7 與 N-6 都是這輪才補進來的，正說明原掃描有盲區）。
要有回歸意義的分數，下輪仍須重跑完整體檢。

### 加分歸因（教訓 4：分數上升不等於問題變少）

| 性質 | 幅度 | 說明 |
|------|------|------|
| **真實改善** | 約 +0.55（九面向） | 安全性 +1.6、文件 +2.5、序列化 +0.7、公開 API +0.5、效能 +0.8、並行 +0.8 —— 對應實際落地的修正 |
| **扣掉誤報** | 約 +0.06（九面向） | 散落類別 +0.5 **完全不是改善**：D-1 整項是誤報，原本的 7.0 有一部分建立在不存在的問題上 |
| **測量深度增加後仍持平** | — | 測試品質 8.5 → 8.5：N-6 推翻了「20 處 `[Collection]` 已處理」這個前提（實際只涵蓋寫入端），發現缺陷與修掉缺陷相抵 |

### 為什麼幾個面向沒動

架構分層（A-1 / A-3）、相依分層（A-2 / A-4）、維護性（M-1 / M-2）三個面向的扣分項
**本輪一項未動**，分數照舊。這三者都屬「要改設計或大範圍機械替換」，不在本輪範圍。
