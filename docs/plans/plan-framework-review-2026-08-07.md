...
| # | 項目 | 位置 | 說明 |
|---|------|------|------|
| **S-3** | `ILoginAttemptTracker` 有完整實作但**從未註冊** | `src/Bee.Hosting/BeeFrameworkServiceCollectionExtensions.cs:246-249`（只有註解） | `Login` 是唯一可匿名觸達的憑證驗證面。預設容器不註冊 → `tracker` 恆 null → 三處 lockout 邏輯全部短路。**開箱即用的部署完全無帳號鎖定**。實作躺在 repo 裡且無相依。修法：`TryAddSingleton<ILoginAttemptTracker, LoginAttemptTracker>()` 一行 |
...
| **N-3** | `DbProviderRegistry` / `DbDialectRegistry` 用裸 `Dictionary` | `src/Bee.Db/Manager/DbProviderRegistry.cs:16-17`、`DbDialectRegistry.cs:16` | production 啟動後唯讀，但**測試中不是**：`DbProviderRegistryTests` 平行呼叫 `Register`（含 `Remove`），其他測試同時 `Get`。resize 期間並行讀 → `IndexOutOfRangeException` / 無限迴圈。典型「本機綠、CI 紅」根因。修法：改 `ConcurrentDictionary`，零行為變更 |
| **N-4** | `ClientDefineAccess` 快取是裸 `Dictionary`，從 thread-pool 並行進入 | `src/Bee.Api.Client/ClientDefineAccess.cs:29,88-107` | XML doc 宣稱「Concurrent reads of the same key share a single in-flight request」，但 `TryGetValue` + 索引賦值非原子。觸發源：`ListView.cs:291,371` 的 fire-and-forget + `ConfigureAwait(false)` 續行落 thread pool。修法：`ConcurrentDictionary` + `GetOrAdd` |
...
| **Z-1** | 序列化 analyzer 只掛 3 個專案，gate 沒接滿 | 掛載：`Bee.Business` / `Bee.Api.Core` / `Bee.Definition`；未掛：`Bee.Api.Contracts`（3 個 wire DTO）、`Bee.Db`（5 個框架集合） | BEE4002–4006 對這兩個專案靜默（僅 BEE4001 跨 assembly）。`Bee.Db/DbParameterSpecCollection.cs:18,33` 是全 repo 唯一的多重 public `Add` 違規，正因未掛 analyzer 而未被擋。修法：加 `ProjectReference`，或提到 `Directory.Build.props` 統一注入 |
...
| # | 項目 | 位置 | 說明 |
|---|------|------|------|
| **D-1** | 上輪 14 項死碼清單只清了 10 項 | — | 已清：`IEnterpriseObjectService`、`EnterpriseObjectService`、`InitializeOptions`、`ApplicationType`、`SysFuncIDs`、`VersionFiles`、`DefaultBoolean`、`NotSetBoolean`、`SystemActions.GetLocalDefine`/`SaveLocalDefine`。**未清 4 項**：`TreeNodeIgnoreAttribute`、`IDefineField`、`IElementCapabilityResolver`、`CheckPackageUpdate`/`GetPackage` 全棧（12 檔） |
| **D-2** | `PermissionBindingValidator` 看似防護、production 零呼叫 | `src/Bee.Definition/Settings/Permission/PermissionBindingValidator.cs` | 驗證 `PermissionModelId` / `ScopeRole` 綁定正確性，只有測試呼叫 7 次。**權限綁定錯誤在 runtime 不會被攔下**。建議接進定義載入或 `Bee.Cli` validate 指令（與 `Bee.Analyzers` 那條線是同一件事的兩種實作） |
...
| **D-4** | `IUIViewService` 抽象縫 production 零實作 | `src/Bee.UI.Core/IUIViewService.cs`、`ClientInfo.cs:169,354` | 唯一實作是三個測試 fake。所有實際 head 走另一個多載 → `ClientInfo.UIViewService` 恆 null。`ClientInfoInitializeTests` 註解自稱「補強覆蓋率」，4 個測試全為執行這條死路徑 |
