...
JS 端 client 採用**文件內範例**形式（snippet 嵌入整合指引），不打包為獨立 NPM 套件。
...
- DTO codegen — 範例文件手寫 TypeScript interface，數量少、變動低

**升級到 NPM 套件的觸發條件**

當以下任一條件成立時，把 snippet 抽出為獨立套件（如 `@bee/api-client`）：
- JS 前端 ≥ 3 個專案使用，手動同步 snippet 已開始出包
- 外部開發者要直接整合（非內部專案）
- DTO 型別手動同步已發生過 2 次以上對應錯誤（此時連帶評估 codegen）

升級時 snippet 本身可直接搬入套件，無架構改寫成本。
