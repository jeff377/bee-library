...
**屬性移除**（Phase 4-6 全數完成；最終隨 Phase 6 `BackendInfo.cs` 刪除）：
- [x] `BackendInfo.DefineAccess`
- [x] `BackendInfo.SessionInfoService`
- [x] `BackendInfo.ApiEncryptionKeyProvider`
- [x] `BackendInfo.LoginAttemptTracker`
- [x] `BackendInfo.AccessTokenValidator`
- [x] `BackendInfo.BusinessObjectFactory`
- [x] `BackendInfo.CacheDataSourceProvider` / `BackendInfo.EnterpriseObjectService`

**Bee.Db DML helpers 重構**（Phase 4 完成）：
- [x] 5 個 `*FormCommandBuilder` 移除 `(string progId)` ctor，保留 `(FormSchema)` ctor
- [x] `IDialectFactory.CreateFormCommandBuilder(string)` 介面方法 + 5 個 DialectFactory 實作移除
- [x] `SelectContextBuilder` ctor 接 `IDefineAccess`；`SelectCommandBuilder` 對應調整、傳遞
- [x] `TableSchemaBuilder` ctor 接 `IDefineAccess`；`DatabaseRepository` 對應調整
- [x] 對應測試遷移（含刪除測 `(string progId)` ctor 的測試方法）
...
### Phase 6：移除 BackendInfo 空殼 — ✅ 已完成
**目標**：刪除已縮減至空殼的 `BackendInfo` 類別 + 清理殘留文字引用 + 同步 active 文件。

實際範圍與原始描述差異：執行前盤點發現 `BackendConfiguration.cs` **並非空殼** —— 它仍承載 4 個 XML 反序列化欄位（`SecurityKeySettings` / `Components` 由 `AddBeeFramework` active 消費；`LogOptions` / `ApiKey` 死碼）。`BackendConfiguration` 仍是 `SystemSettings.xml` 的 `<BackendConfiguration>` 反序列化根節點，故 Phase 6 採方案 A 保留它，不強拆。

實際落地：

- 刪除 `src/Bee.Definition/BackendInfo.cs`（空殼）
- 刪除 `BackendConfiguration.ApiKey` 死欄位（0 production caller，3 個測試改用 `SecurityKeySettings.MasterKeySource.Value` 作為 round-trip 識別字串）
- sweep src/tests 殘留文字引用（`ApiClientInfo` / `SystemBusinessObject` / `IDatabaseSettingsProvider` docstring + `StaticApiEncryptionKeyProviderTests` 註解）
- 刪除 `BackendInfoTests.cs`（Phase 4 殘留重複測試，已被 `DefineAccessDatabaseSettingsProviderTests.cs` 完整 cover）
- sweep active 文件（`development-cookbook` / `development-constraints` 整節重寫 / `database-settings-guide` 13 處 sweep / `Bee.Definition/README` 含 zh-TW，全部改為 DI 等價）
- ADR-003 加 Superseded 註記；ADR-010 加等價對照註記
- `BackendConfiguration.LogOptions` 死欄位明確不刪（XML fixture 含節點 + 外部部署相容風險）

**未在此 phase 處理（明確排除）**：
- `BackendConfiguration.cs` 整類刪除（仍是 XML 反序列化 root）
- `BackendConfiguration` 解構為 `IOptions<T>`（範圍過大）
- 補 server 端 API Key 比對邏輯（獨立議題）
...
- [x] `grep -r "BackendInfo" src/ tests/` 結果為 0（含檔案本身，Phase 6 完成）
- [x] N/A：本 repo 無 `samples/` 目錄（v5.0 完成時主計畫成功標準的此條目改為「使用此框架的下游應用需以 `AddBeeFramework` 啟動」，由外部消費者文件涵蓋）
- [x] `dotnet test` 不依賴 `GlobalFixture` 或 `[Collection("Initialize")]`（Phase 5 完成）
- [x] CI 通過時間相較 v4.x 不退步（Phase 5 完成後本機 wall 1m47s / user 4m49s = ~2.7x parallel speedup，2749 tests；Phase 7 進一步移除 `[Collection("DbConnectionState")]` 後並行範圍再擴大）
- [x] `docs/` active 文件中無 `BackendInfo` 字樣（除 `docs/plans/` 歷史紀錄 + `docs/adr/` 加註記引用 + `docs/archive/` 封存；Phase 6 完成）
