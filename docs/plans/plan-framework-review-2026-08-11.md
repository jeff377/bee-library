...
| 階段 | 範圍 | 項目數 | 狀態 |
|------|------|--------|------|
| P0 | 發版阻擋項（安全 / 發版正確性） | 3 | 📝 擬定中 |
| P1 | 閘門可靠性與已證實的功能缺陷 | 11 | 📝 擬定中 |
| P2 | 結構、效能、一致性 | 14 | 📝 擬定中 |
| P3 | 文件漂移與低風險清理 | 12 | 📝 擬定中 |
| P4 | 觀察／待裁決 | 9 | 📝 擬定中 |
...
| # | 項目 | 理由 |
|---|------|------|
| **SEC-1** | 型別白名單泛型繞過（**已實測證實，未認證可達**） | 安全缺陷隨版本發布 |
| **REL-1** | `AssemblyVersion` / `FileVersion` 漏升（C-5 回歸） | NuGet 發布後不可回收，已錯過一次 |
| **REL-2** | CHANGELOG 完全未寫（11 項應記載，含 **4 筆破壞性**，其中一筆是登入行為變更） | 發版必要條件 |
