...
**狀態：✅ 已完成（2026-08-12）** —— 全部項目均已處理（修正 / 明示裁決不修 / 記錄阻擋），
5 條刻意遞延與 2 筆待查 flaky 見〈遞延與待查〉。

> **對帳更正（2026-08-12）**：本檔一度標為完成時，「🟠 強烈建議」那組的 **X-4 / X-5 其實尚未處理**——
> 當時只做了同組的 M-1 / GATE-1 / GATE-2。兩項已於 `fcb49f36` 之後補上。
> 教訓：階段表（P0–P4）不涵蓋「建議」那三組，只看階段表對帳會漏。
...
| # | 面向 | 上輪 | 本輪掃描 | **修正後** | 變化 | 剩餘扣分 |
|---|------|------|---------|-----------|------|---------|
| 1 | 架構分層 | 8.6 | 8.7 | **9.0** | ▲0.3 | **A-5**（資料遷移，記錄阻擋）、**A-3**（使用者暫緩）。A-4 已修、A-2 已加閘門 |
| 2 | 相依分層 | 9.0 | 9.2 | **9.4** | ▲0.2 | DEP-1 明示不修（`ProjectReference` 傳遞可見性使 csproj 非可信來源，接受）。X-7 新增 `Bee.Api.Contracts` 閘門 |
| 3 | 安全性 | 8.6 | 7.0 | **8.7** | ▲1.7 | **SEC-3 硬失敗**（遞延 major）、**帳號鎖定的 IP 維度**（需 client IP 流到 BO 層）。SEC-1/SEC-2 已修，`EXEC` 跳脫、三處空 catch、工廠面 `isLocalCall` 已修 |
| 4 | 維護性 | 8.5 | 8.5 | **9.4** | ▲0.9 | 僅餘 BO 建構子的 `isLocalCall` 預設（約 80 呼叫點）。M-1/M-2/M-3 全落地；`src/` ≥500 行檔案數 **0** |
| 5 | 散落／不必要類別 | 7.5 | 7.5 | **9.3** | ▲1.8 | 無實質殘留。D-6/D-7/D-8/D-9 全清，X-4 收掉 5 個公開破口 |
| 6 | 序列化一致性 | 9.2 | 8.8 | **9.5** | ▲0.7 | 無實質殘留。GATE-2、SEC-1 序列化面、`WireValueCode` 判別碼釘死、drift 反方向檢查 |
| 7 | 公開 API 表面 | 9.0 | 8.5 | **9.2** | ▲0.7 | X-5 裁決不移除（`Login` 有內部呼叫端）。REL-1、X-4 已落地 |
| 8 | 測試品質與覆蓋 | 8.5 | 8.2 | **9.0** | ▲0.8 | **2 筆 flaky 未結案**（`ExecFunc_Hello`、`CrossNode_PostgreSQL`）。GATE-1/2/3、TEST-1/2/3、T-2(7 處)、T-3 全落地 |
| 9 | 文件漂移 | 8.5 | 7.5 | **9.2** | ▲1.7 | 無實質殘留。DOC-1/2/3、REL-2、X-6 已修；規則層本輪另更正 4 處失真敘述 |
| 10 | 效能／熱路徑 | 6.8 | 6.5 | **8.6** | ▲2.1 | P-2(b) 明示接受（欄名逐列重複）。PERF-1 4.7×、P-4 23.4→0.08 µs、PERF-3、P-2(a) 已修；P-3/PERF-2 實測後不修 |
| 11 | 並行與全域狀態 | 7.8 | 7.6 | **9.1** | ▲1.5 | 無實質殘留。CON-1~CON-5、N-5、A-4 全落地 |

**九面向平均 8.32 → 9.19**（不含效能、並行）；**八面向 8.40 → 9.22**；**十一面向 8.09 → 9.13**。

> **評分方法與本輪掃描一致**：扣分只看「現在還開著什麼」，不獎勵「修了多少」。因此
> **A-5 / A-3 這類記錄阻擋或使用者暫緩的項目仍然扣分**——它們沒有被解決，只是被理解了。
> 同理，2 筆未結案 flaky 讓「測試品質」停在 9.0 而非更高：閘門補得再密，測試套件本身的
> 可信度有缺口就是缺口。
>
> **三個面向沒有滿分的共同理由**：安全性（8.7）與架構分層（9.0）各有一項需要更大改動才能
> 收掉（啟動硬失敗需 major + opt-out、`FileDefineStorage` 搬家需舊型別名相容對映），
> 效能（8.6）則是 `P-2(b)` 屬明示接受的設計取捨而非缺陷。
...
| P2 | 結構、效能、一致性 | 16 | ✅ **已完成**（2026-08-12）。修正：P-2(a) / CON-2 / CON-3 / CON-4 / A-4 / N-5 / P-4 / PERF-3；加閘門：**A-2**；評估後不修：DEP-1 / P-3 / PERF-2；明示接受：P-2(b) / N-1；**遞延：A-3**（使用者暫緩）、**A-5**（資料遷移，見阻擋說明） |
| P3 | 文件漂移與低風險清理 | 13 | ✅ **已完成**（2026-08-11，13 項全數落地） |
| P4 | 觀察／待裁決 | 15 | ✅ **已完成**（2026-08-12）。M-1 / M-2 / M-3 / D-6 / D-7 / D-8 / D-9 / T-2 / T-3 / X-6 / N-2·N-3 / Z-3·Z-5 / CON-5 / X-7 全數落地；**SEC-4~10 部分**——三處空 catch、`EXEC` 跳脫、工廠面 `isLocalCall` 已修，**帳號鎖定的 IP 維度遞延** |

### 新發現（X-5 裁決連帶翻出，非原體檢項目）

**`rules/definition.md` 把 `Login` 誤列為「只給 client、不放介面」的例子。** 它有真實的內部
呼叫端：**背景作業會以某身份登入建立連線**，再模擬該使用者登打表單或執行作業。
規則的**判準本身沒錯**（「會不會有另一個 BO／背景作業／排程經 `BoFactory` 拿到後呼叫它？」），
錯的是漏算了背景作業這類呼叫端。規則與 `ISystemBusinessObject` 的 XML doc 均已更正。

> **過程中我把方向理解錯了一次，值得記。** 我曾據「API 開放的方法一般也會出現在介面上」
> 改寫該節，並量出「24 個 `[ApiAccessControl]` 方法只有 7 個在介面上」當成 17 處落差。
> 那個前提已由使用者撤回：**API 是給外部呼叫、BO 介面是給內部呼叫，兩者各自獨立、沒有硬性
> 規定**。所以 24 vs 7 不是落差，只是兩條獨立軸線的自然結果——那份「17 處待裁決」的清單
> 已隨之刪除。教訓：量測之前要先確認自己用的判準是對的，否則量出來的是一份看似精確的雜訊。

### 遞延與待查（本輪刻意不做，各有理由）

| # | 項目 | 為何遞延 |
|---|------|---------|
| **A-5** | Domain Core 夾帶 1,119 行檔案 IO | **是資料遷移不是重構**：`BackendDefaultTypes.DefineStorage` 的型別名寫在每個既有部署的 `SystemSettings.xml` 裡，搬組件會讓那些 XML 指向不存在的型別。要做必須配套舊型別名相容對映 |
| **A-3** | `Bee.UI.Core/Permissions/` 位置 + Blazor 無權限降級 | 使用者裁決暫緩（Blazor 尚未實作前端） |
| **SEC-3** | 啟動時 API key gate 失效改為硬失敗 | 會讓每個尚未發出金鑰的部署一升級就起不來；應放 major 版並附 opt-out |
| **SEC-4~10** | 帳號鎖定改以 `(userId, IP)` 為 key | client IP 目前只存在於 `ApiServiceController` 的異常稽核，沒有流到 BO 層；加上 `ILoginAttemptTracker` 介面與設定都要改。是功能不是 hardening 小項 |
| **—** | BO 建構子的 `isLocalCall = true` 預設 | 本輪只改工廠面。建構子那半約 80 個呼叫點（含 `apps/` 與 `samples/` 的子類 `: base(...)`），且直接 `new` 一個 BO 比呼叫工廠更像刻意行為 |

**兩筆 flaky 未結案**（判準：不同 commit 的**首次**執行是否都紅）：
`ApiAspNetCoreTests.ExecFunc_Hello_ReturnsNotNull`（嫌疑：`SharedDatabaseState` seed 的跨行程爭用）、
`DbDefineCacheInvalidationTests.CrossNode_PostgreSQL`（兩個待查方向見下文）。
...
| **X-5** ✅ | `Login` 是否留在 `ISystemBusinessObject` 定案 | **裁決：留在介面上。** 它有真實的跨 BO 消費端——背景作業以某身份登入建立連線後模擬該使用者作業。連帶發現 `rules/definition.md` 該節**方向整個寫反**（見〈新發現〉），已改寫 |
