...
>
> **前置依賴**：[plan-dateonly-type.md](plan-dateonly-type.md)（區分 `DateTime` / `DateOnly` 型別）。
> 該 plan 為獨立議題、可先行出貨；本 plan 的 D4 倚賴它提供「日曆日欄位靠 CLR 型別自我描述」，
...
| CLR 型別 / wire 標記 | `FieldDbType.Date` 與 `DateTime` 在 CLR 層被抹平為 `typeof(DateTime)`，連帶使 wire 的日曆日欄位標成 `DateTime` | **由前置 plan（[plan-dateonly-type.md](plan-dateonly-type.md)）處理**；完成後日曆日欄位靠 CLR 型別自我描述 |
...
| 儲存格 CLR 型別 | 處理 | 需要 schema？ |
|----------------|------|-------------|
| `DateOnly` | **絕不轉** | ❌ 型別自身即語意 |
| `DateTime` | 預設視為 Instant，轉換 | ⚠️ 僅在需要 `Local` 覆寫時才查 schema |

因此**沒有 `FormSchema` 的場景（報表 / AnyCode）同樣適用**：日曆日欄位靠 CLR 型別自我描述、
絕不誤轉；`DateTime` 欄位走 Instant 預設。schema 只為少數 `Local` 欄位而查——
而報表輸出通常不含此類欄位。

> 日曆日欄位如何成為 `DateOnly`，由前置 plan 的兩路徑規則決定：定義驅動的查詢由框架轉換，
> BO 自寫 SQL 則由 BO 於取回 `DataTable` 後自行轉換（共用同一個 helper）。
> **兩條路徑產出的 `DataTable` 對 Connector 是同構的**——Connector 只看儲存格 CLR 型別，
> 不需知道欄位型別由誰決定。
...
> **與前置 plan 的分工**：`DateOnly` 語意由 CLR 型別承載（見 [plan-dateonly-type.md](plan-dateonly-type.md)），
> **不需要標記也不需要 schema**。`DateTimeSemantics` 標記因此只剩一個用途——把某個 `DateTime`
> 欄位覆寫為 `Local`。這也是前置 plan 的價值所在：它把三種語意裡最危險的一種（DateOnly 誤轉）
> 從「靠標記 + 靠 schema」降級成「靠型別」。
...
- DB 讀出的 instant 值統一 `SpecifyKind(Utc)`；`DateOnly` 語意欄位維持 `Unspecified`，不套 UTC。
...
1. **逐欄判斷，不可全表套用**（依 §1.1）：`st_session` 等已是 UTC 的欄位不可再轉；
   `DateOnly` / `Local` 語意欄位不動；只轉 `Instant` 語意欄位。
...
| **P1** | D5 語意標記：`DbField` 新增 `DateTimeSemantics` 屬性、定義層覆寫、三棲序列化相容（新增欄位不可破壞既有定義檔反序列化）。範圍僅涵蓋 `Local` 覆寫——`DateOnly` 由前置 plan 的型別變更承擔。 |
...
- **`DateOnly` 誤轉**（D5）——前置 plan 完成後由型別阻擋，殘餘風險落在 `DateTime` 欄位
  被誤標為 Instant 的情況；測試仍需專門覆蓋跨日邊界。
- **前置 plan 未完成就實作 D4**——`DateOnly` 欄位在 schema-less 場景會被當成 Instant 轉時區。
  D4 的實作必須等前置 plan 落地。
