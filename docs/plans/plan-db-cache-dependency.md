...
2. `BackendConfiguration` 暴露輪詢間隔的設定鍵名與是否可停用 poller（純單節點 + 全框架寫入時可關）。
   ...
   - ✅ **§2a/§2b 維持增量設計**（多租戶通知表列數大，避免每輪全表掃）：`sys_update_time` 增量抓取 + `margin` 餘量 + highWater 游標 + `version` 冪等，與原設計一致；通知表加 `ix_{0}_update_time` 索引讓增量 `WHERE` 走索引（否則仍全表掃）。
   - ⚠️ **跨方言 `DateTime` 參數雷的解法**：highWater 門檻**不以 `DateTime` 參數綁定**（Npgsql 把 `DbType.DateTime` 解析為 `timestamptz` 要求 `Kind=Utc`，與 naive `timestamp` 欄衝突；其他 provider 亦各有差異），改為**格式化為 ISO-8601 字串參數 + 各方言在 SQL 內 `CAST`/`TO_TIMESTAMP`**（SQL Server `datetime2`、PG `timestamp`、MySQL `DATETIME(6)`、Oracle `TO_TIMESTAMP` 指定格式遮罩、SQLite 文字比較）。baseline 的 `MAX(update_time)`／空表 naive-now 為純 SELECT 無參數，避開該雷。`MarginSeconds` 設定保留。
