...
| 層 | 名稱 | 說明 |
|----|------|------|
| marketplace repo | `jeff377-plugins`（public） | 已建於 https://github.com/jeff377/jeff377-plugins |
| plugin | `plan-workflow` | 只裝 plan 工作流，**不含** PR / 發版 / code style |
| skill | `plan-write` | 呼叫為 `/plan-workflow:plan-write` |

> **命名變更**：原定與 repo 名一致用 `claude-plugins`，但 `claude plugin validate` 會擋 —— 官方保留
> `claude-*` 字樣、判為冒充官方 marketplace。改用 `jeff377-plugins`（個人 namespace）驗證通過。
...
```
/plugin marketplace add jeff377/jeff377-plugins
