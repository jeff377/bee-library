...
| [客製 BO 與 Repository 類別](plan-customization-business.md) | ✅ 已完成（**B1、B2 全數落地**，2026-08-05） | 以租戶客製的類別整個換掉套裝的 BO / Repository。補完兩個會靜默出錯的缺口：`ProgramItem` 改屬性級繼承（只換 BO 不再連帶打掉套裝的 Repository，並加反射防護測試釘住新屬性）、一般 progId 的型別解析失敗改為降級 + error log（訊息標示 progId、型別名與宣告層）。ADR-016 的粒度表同步加註修訂 |
| [業務邏輯 plugin](plan-customization-plugin.md) | ✅ 已完成（**G1–G4 全數落地**，2026-08-06） | 不換整個 BO，只在既有流程的四個時點（`BeforeSave` / `AfterSave` / `BeforeDelete` / `AfterDelete`）掛一段客製程式碼，每次操作一個實例、可跨時點傳遞狀態。新增 `PluginSettings.xml`——**第一個可寫的客製定義**，兩層相加而非取代，經 `LocalOnly` API 維護並於儲存時驗證每個綁定型別 |
