...
| [BO 擴充點的交易邊界契約](plan-bo-transaction-contract.md) | 📝 擬定中（**決策已定案**，2026-08-05） | 把 `Save` / `Delete` 六個可覆寫子方法的交易邊界寫成明文契約（只有 `Do*` 在交易中），含 TOCTOU 空窗、稽核與資料不原子、After 失敗時資料已提交三個後果。**純文件、零行為變更**。裁決：其他 BO 方法不拆三段、交易不上提到 BO 層 |
