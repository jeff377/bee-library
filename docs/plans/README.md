...
| 計畫 | 狀態 | 說明 |
|------|------|------|
| [設定檔健檢（2026-08-12）](plan-config-audit-2026-08-12.md) | 📝 擬定中（2026-08-12） | 常駐設定語料（`CLAUDE.md`／`rules/`／skills／memory）首次基線健檢。P0 為 plugin 名失效與 AOT 結論跨檔矛盾；另清除設定內的清點數字（必漂且對 agent 無用）、把推導內容搬按需區。階段 1（P0）純字串修正可先交付；階段 3 需先裁決是否新建行動端 gotchas 檔 |
