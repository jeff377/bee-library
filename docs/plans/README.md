...
| [API Key 存放機制與預設驗證強化](plan-api-key-store.md) | 🚧 進行中（2026-07-30） | `X-Api-Key` 的存放位置與預設驗證行為：`st_api_key` + 兩段式金鑰 + 雜湊存放 + 相容閘門。**階段 1（存放模型與驗證 + 產生金鑰 BO 方法）已完成**；階段 2（呼叫端識別）／3（管理與輪替）／4（用戶端存放）待做。僅「相容模式的長期處置」待決 |
