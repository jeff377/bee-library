...
| 計畫 | 狀態 | 說明 |
|------|------|------|
| [bee-oauth2 改名為 Polhem.OAuth2 並移至 polhem-dev](plan-polhem-oauth2.md) | ✅ 已完成（2026-09-15） | 新開 `polhem-dev/polhem-oauth2`（不帶舊歷史），先純改名並拿掉 `Bee.Base`，再做四項功能修改（state 解密健壯性、System.Text.Json、拿掉 WebView2 改系統瀏覽器、net10.0），套件由五個減為三個（桌面流程併入核心），改完才首發 1.0.0 並凍結舊套件。首發前依健檢結果修掉全部發現並定型公開 API（階段 6b）。程式碼風格依 bee-library 現行設定；共同維護的部分（程式碼、測試、commit）一律英文，公開 `.md` 中英雙語。**同時是 bee-library 改名另開的演練**，結果回寫 future-work；階段 1a／1b／2–6／6b／7–9 |
| [個人文件移出 docs/，改放獨立的個人目錄](plan-personal-docs-directory.md) | 📝 擬定中（2026-09-11） | `docs/blogs/`、`docs/internal/` 移到 gitignored 的個人目錄並以 private repo 存放；非個人文件都留在 `docs/`。**延後至 2026 鐵人賽 Day 30 發佈完成後**；plan 是否也移入列為未來考慮。由多語系 plan 的原階段 4 拆出 |
