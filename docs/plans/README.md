...
| 計畫 | 狀態 | 說明 |
|------|------|------|
| [深分頁成本與跨引擎壓測的隔離](plan-paging-and-oracle-getlist.md) | 🚧 進行中（2026-09-08） | 階段 A（Oracle 的 FormSchema 請求全數失敗）**已修**——根因是 `BindByName` 預設 false 導致登入時參數錯位，不在 GetList 的 SELECT 上。深分頁成本三家兩輪皆重現（`OFFSET 94,950` 下 p50 慢 15–19 倍），**待決定值不值得處理**——keyset pagination 的代價是不支援跳頁，屬產品決策。Oracle 的數據待補：它的連線字串沒有 `{@DbName}`，壓測前綴無從隔離，需先備妥專用 schema |
