...
| 計畫 | 狀態 | 說明 |
|------|------|------|
| [數量／重量欄必須綁定 `UnitField`](plan-unit-field-required.md) | 🚧 進行中（2026-09-11） | 標成數量／重量的欄位必須綁 `UnitField`（不需要單位的用一般數值），執行期對齊多幣別作法、公司不再決定數量位數。**階段 0 裁定已完成**，階段 1（框架）與階段 2（DefineEditor 設計期檢查）待動工。外部對照見 `docs/repo-ops/uom-decimals-prior-art.md` |
