...
**狀態：✅ 已完成（2026-09-04）** —— **P0 / P1 / P2 / P3 / P4 的條目全數處理完畢**（71 項已修、
`T-2` 與 `CON-6` 查證後撤回）。

**但「處理完畢」不等於「零殘留」，以下五處仍開著，各自記明理由**：
`Z-13`（32-bit 才可能撕裂，不建議改）／`Z-15`（0 消費者 public API，依規則保留）／
`A-5` + `A-3`（使用者裁決遞延，屬資料遷移非重構）／`DEP-1`（明示不修，且已驗證閉包全空）／
**`T-6` 殘留**（poll cursor 是全表的；修它要為了測試在 production 介面上加 API，
flake 又重現不出來，判定維持觀察）。
...
> **⚠️ 這是自評，不是重新掃描。** 評分由**執行修正的同一個 agent** 依「扣分項是否關閉」推算，
> 沒有重跑一次十一面向的平行子代理審查。它證得了「原本點名的缺陷已關閉」，**證不了「沒有新的缺陷」**
> —— 後者只有獨立重掃才算數。要拿一個對得起上表的數字，應重跑 `bee-framework-review`。

| # | 面向 | 本輪 | 修正後 | 變化 | 扣分項現況 |
|---|------|------|--------|------|-----------|
| 1 | 架構分層 | 8.8 | **9.0** | ▲0.2 | ARCH-1~4 全關；**開著 A-5 / A-3**（使用者裁決遞延，上輪 9.0 時同樣遞延） |
| 2 | 相依分層 | 9.0 | **9.4** | ▲0.4 | GATE-1 關；DEP-1 明示不修且已驗證閉包全空 |
| 3 | 安全性 | 8.2 | **8.7** | ▲0.5 | SEC-1/2/3 關，另加 SEC-4/5/6 與 BEE1008；**三項遞延仍在** |
| 4 | 維護性 | 9.2 | **9.4** | ▲0.2 | M-1/2/3 全關 |
| 5 | 散落／不必要類別 | 9.2 | **9.3** | ▲0.1 | D-1~D-4 全關；Z-15 依「0-caller 公開 API 保留」留著 |
| 6 | 序列化一致性 | 8.7 | **9.5** | ▲0.8 | S-1~S-10 全關（含 DataTable 精度與兩道恆真閘門） |
| 7 | 公開 API 表面 | 8.6 | **9.2** | ▲0.6 | API-1~API-5 全關；analyzer 出貨基準回填並經負向驗證 |
| 8 | 測試品質與覆蓋 | 8.8 | **9.0** | ▲0.2 | T-1/3/4/5/7/8 關、T-2 撤回；**T-6 殘留**（poll cursor 全表，維持觀察） |
| 9 | 文件漂移 | 7.6 | **9.3** | ▲1.7 | DOC 全關、17 條漂移、239 處 `<c>`→`<see cref>`、README 五處失實宣稱 |
| 10 | 效能／熱路徑 | 8.0 | **8.8** | ▲0.8 | PERF-1（批次寫入）、PERF-2（stream 讀 body）皆關並附實測數據 |
| 11 | 並行與全域狀態 | 8.8 | **9.2** | ▲0.4 | CON-1~CON-5 全關、CON-6 撤回 |

**十一面向 8.63 → 9.16**（▲0.53），略高於上輪修正後的 9.13。

**這個「略高」是本輪唯一值得看的數字，其餘都只是回補。** 它來自新增的可執行閘門
（分層硬約束、`Messages` 命名空間、`isLocalCall` 預設、`Bee.Base` 不得參考
`System.Net.Http`、cache-notify 基準、analyzer 出貨紀錄）—— 這些位置在上輪拿 9.13 時
**同樣是乾淨的，但沒有任何東西在守它**。分數一樣，可回歸性不一樣。

### 殘留（五處，全部是已記錄的決定而非未知）

| 項目 | 為何不動 |
|------|---------|
| `A-5` / `A-3` | 使用者裁決遞延。A-5 的搬遷需舊型別名相容對映，屬資料遷移 |
| `DEP-1` | 明示不修；已驗證 `OUTSIDE_CLOSURE` 全空，plan 註明下輪不需再提 |
| `Z-13` | 只在 32-bit runtime 可能撕裂，最壞結果是多掃一次（無害） |
| `Z-15` | 0 消費者的 shipped public enum，依「0-caller 框架公開 API 保留」應留 |
| `T-6` 殘留 | poll cursor 是全表的；修它要為了測試在 production 介面加 API，而 flake 連跑 20/20 + 12 輪皆未重現 |

### 下輪要帶走的一句

本輪反覆出現的形狀是**「一道防護的實際涵蓋範圍比它看起來的窄，而沒有任何東西會說出這件事」**。
它已落成常駐規則（`~/.claude/rules/code-style.md`：帶絕對語氣的宣稱必須指得出執行它的機制）。
**這份 plan 自己的頂部狀態列就犯過一次** —— 原本寫「P0–P4 全數關閉」而 `T-6` 標的是部分完成，
2026-09-05 修正。
...
| 階段 | 範圍 | 項目數 | 狀態 |
|------|------|--------|------|
| P0 | 已出貨功能遠端不可用 | — | ✅ **已完成**（2026-09-04，P0-1 修正 + 新增保留字 progId 建構閘門，負向驗證通過） |
| P1 | 授權邊界、跨語言 wire 正確性、實測效能、閘門可靠性 | — | ✅ **已完成**（2026-09-04，11 項全數落地，皆經負向驗證或實測） |
| P2 | 結構、並行、一致性 | — | ✅ **已完成**（2026-09-04，含 D-1 的 helper 下沉；T-2 查證後不成立） |
| P3 | 文件漂移與低風險清理 | — | ✅ **已完成**（2026-09-04，全數關閉；`T-2` / `CON-6` 查證後撤回，`T-6` 的 flake 未重現但同元件的 UTC 基準 bug 已修） |
| P4 | 觀察／待裁決 | — | ✅ **已完成**（2026-09-04，可修項全數關閉；`Z-13` 建議不改、`Z-15` 依 0-caller 公開 API 規則保留、`A-5` / `A-3` 經使用者裁決遞延、`DEP-1` 明示不修） |
...
| **API-4** ✅ 已修（2026-09-04） | 236 處 `<c>Xxx</c>` 藏著本 solution 內、cref 解析得到的型別／成員（112 處同組件、124 處向下相依組件；101 個相異識別字）。`code-style.md` 明訂這種情況一律用 `<see cref>` 交給編譯器把關（CS1574），而 `check-xmldoc-refs.sh` **只驗「這個名字還存在嗎」，不驗「這裡本來就該用 cref」** —— 腳本檔頭自己寫的優先序沒有任何機制在執行。基準 `227daa70` 量得 245、HEAD 259，**沒在收斂**。建議先改同組件那 112 處（機械性、零風險），並把反向檢查加進腳本<br>**已落地**：**239 處**（144 檔）轉為 `<see cref>`，由編譯器把關。轉換法：全部轉 → build → **依編譯器回報的「檔案+行號」精準退回**解不到的 → 迭代至收斂。第二輪對不同 namespace 的改用**完全限定名**（不能為了 cref 加 `using`，那會觸發 IDE0005），同專案內殘留因此從 88 降到 26 —— 其中 17 處是檔名（`.xml` / `.razor`）本來就該用 `<c>`。<br>**反向檢查已加進 `check-xmldoc-refs.sh`**：同專案內宣告的型別若還用 `<c>` 就報出來，排除檔名型，allowlist 目前只有 `WhereBuilder`（重名型別，完全限定名有多個候選）。負向驗證：塞一個 `<c>IPValidator</c>` 進 `SysInfo.cs`，確實被抓到。<br>**⚠️ 順帶抓到我自己這輪引入的 4 筆違規**：`FileBufferingReadStream`（PERF-2）、`GetIndexsCommandText`（D-4）、`LOCALTIMESTAMP`（T-6）、`NO_BACKSLASH_ESCAPES`（SEC-3）—— 前一道檢查本來就擋得住，但**我從沒跑過這支腳本**。已逐筆歸類進 allowlist。這是「閘門存在但沒人跑」的實例，與本輪反覆出現的形狀相同
| **API-5** ✅ 已修（2026-09-04） | 契約介面 5 個集合成員中 3 個曝露具象 `List`/`Dictionary`（`ISaveResponse.AffectedRows`、`ICheckPackageUpdateRequest.Queries`/`Response.Updates`、`IEnterCompanyResponse.Capabilities`），而兩個新的（`Fields`、`ApiKeys`）已用 `IReadOnlyList` —— 慣例已成形，舊的沒回頭收。**注意真實張力**：wire DTO 與 BO 型別**必須**是可寫具象集合（序列化繫結需要），**只有契約介面那一層可以收斂**（介面宣告 `IReadOnlyList`，實作照樣持有 `List`）。`Dictionary` → `IReadOnlyDictionary` 是二進位＋原始碼雙重破壞，**趁 pre-stable 視窗**，錯過要再開一次<br>**已落地**：四個契約成員收斂為唯讀：`ISaveResponse.AffectedRows` 與 `IEnterCompanyResponse.Capabilities` → `IReadOnlyDictionary`，`ICheckPackageUpdateRequest.Queries` 與 `ICheckPackageUpdateResponse.Updates` → `IReadOnlyList`。<br>**序列化零影響**：8 個實作端的具象可寫屬性一字未改，各只多一行**顯式介面實作**（`IReadOnlyDictionary<...> ISaveResponse.AffectedRows => AffectedRows;`）—— 這是 `IListApiKeysResponse.ApiKeys` 與 `IGetChangeDetailResponse.Fields` 早已在用的模式，照抄既有慣例而非自創。`wire-contracts/` 與 `wire-fixtures/` **零 diff**，`bee-connector-js` 不受影響。<br>**破壞面比預想小**：14 檔中破壞性只有契約介面那 4 行，其餘都是加法（淨 +32 / −8）。受影響的只有「拿契約介面當型別、且想寫入那個集合」的外部程式碼 —— 而契約介面本來就全是 `{ get; }`，能取到具象集合並修改它是**當初宣告型別選錯造成的意外能力**，不是設計。
...
| # | 項目 |
|---|------|
| **Z-1** ✅ 已修（2026-09-04） | `UpgradeOptions.Default` 是可變型別的共用 static 單例（`src/Bee.Db/Schema/UpgradeOptions.cs:13,17`）。`AllowColumnNarrowing { get; set; }` + `static Default { get; } = new()`，任何呼叫端寫入就把「允許欄位縮短（可能截斷資料）」開給全 process。repo 內**零寫入點**，故為 API 形狀風險。對照 `MaterializeOptions` 同形狀但用 `init` → 結構上不可變。修法：改 `init`<br>**已落地**：`AllowColumnNarrowing` 由 `set` 改 `init`，`Default` 因此結構上不可變。production 零寫入點、測試用物件初始設定式（`init` 相容）。PublicAPI 基準同步（`.set` → `.init`）。原始碼破壞面：建構後才指派的呼叫端要改用物件初始設定式
| **Z-2** ✅ 已修（2026-09-04） | `CacheDefineAccess.GetDatabaseSettings` 每次呼叫都對快取實例 `DecryptInPlace`（`CacheDefineAccess.Settings.cs:38-43`）。字面上違反「cache 內物件不可異動」，實務上收斂（`Decrypt` 對無 `enc:` 前綴原樣回傳、字串賦值原子、每個呼叫端從頭跑完整解密所以自癒），**目前不是活的 bug**。但它是 repo 內唯一一處「明知在 mutate 快取」卻沒把理由寫在旁邊的地方。建議補 WHY 註解，否則下一個讀者會照抄這個形狀到不收斂的地方<br>**已落地**：補 WHY 註解，並**明列三個收斂條件**（`Decrypt` 對無 `enc:` 前綴原樣回傳 → 冪等；每個呼叫端從頭跑完整解密 → 自癒；字串賦值原子 → 讀者不會看到半寫入值），以及「三者缺一不可」。原本只有結論沒有理由，下一個讀者會把形狀抄到不收斂的地方
| **Z-3** ✅ 已修（2026-09-04） | N-1（`SerializeDefine` 對快取實例序列化）的 `<remarks>` 對損害的描述仍不完整。現行寫「effect is transient」，但實際上：(a) A、B 並行序列化同一實例時 A 的 `NotifyAfter` 會提早結束 B 的狀態 → B 產生的 XML 把空集合輸出成 `<Items />` 而非省略，**這是已送出的錯誤輸出、不是 transient**；(b) 併發讀者不只看到 null，還可能 NRE（四個裸 `!` 點）。不必改行為，但把 remarks 補到與實情相符 —— 這正是「明示接受」該有的樣子<br>**已落地**：`<remarks>` 改寫成與實情相符。原文「effect is transient」低估兩件事：(a) 併發讀者不只讀到 null，四個裸 `!` 點會擲 NRE；(b) 兩個併發序列化不獨立 —— 先完成者清掉狀態，後者把空集合輸出成 `<Items />`，**那份錯誤文件已經送出去了**，不是 transient。不改行為，只讓「明示接受」誠實
| **Z-4** ✅ 已修（2026-09-04） | `ApiCallContext` 從不上 wire，卻有完整 wire 註冊（`WireContracts.Envelope.cs:39-42`）並被發佈進 `wire-contracts/messages.d.ts:80-84`。`RegisteredContracts_AreReachableFromTheClosure`（`ApiErrorInfo` 那道反方向閘門）**結構上抓不到它** —— 閉包是命名空間字串比對，「在 Messages 命名空間」不等於「會上 wire」。連帶把 `format: PayloadFormat`（字串聯集）送進 TS 合約，而信封自己的 `format` 是**數字**，同名不同型。且它是**安全判定型別**（帶 `IsLocalCall`）成為客戶端可具現的反序列化目標（目前不可利用）<br>**已落地**：**這條有兩半，而報告只指出了症狀**：移除 wire 註冊之後 TS 合約**沒有變** —— 產生器不是照註冊清單，而是**按命名空間反射掃**（`WireContractGenerator.Generate` 取 `Bee.Api.Core.Messages*` 下每個公開類別）。根因是 `ApiCallContext` 根本不該在 `Messages` 命名空間：它是 `ApiAccessValidator` 的輸入，由 `JsonRpcExecutor` 每次呼叫建一個，從不上 wire。<br>因此除了移除註冊，也把型別搬到 `Bee.Api.Core.Validator`。TS 合約隨之少掉 `ApiCallContext` **與 `PayloadFormat`** —— 後者的字串聯集（`'Plain' | 'Encoded' | 'Encrypted'`）**只因為它才存在**，而實際 wire 上的 `format` 是數字，同名欄位合約與 wire 互相矛盾。<br>**閘門**：新增 `MessagesNamespaceGateTests` —— `Bee.Api.Core.Messages` 下的公開類別必須都是 `ApiMessageBase` 子類（搬走之後恰好全部符合）。既有那道結構上抓不到，因為它用的正是「在 Messages 命名空間」這個此處要質疑的前提。負向驗證：在該命名空間放一個非訊息公開類別，確實變紅
| **Z-5** ✅ 已修（2026-09-04） | `ApiPayloadOptionsFactory.CreateSerializer` **零 production 呼叫者**（4.27.0 起 `Initialize` 不再呼叫它）。留著會讓人以為「codec 仍是設定值」，而拿它的產物去呼叫 `Initialize(serializer,…)` 會改掉**未宣告 codec 的預設**，靜默打斷所有既有 client。建議移除（公開 API 變更）或在 doc 明寫「這不是設定 codec 的方法」<br>**已落地**：移除零 production 呼叫者的 `CreateSerializer`（`CreateCompressor` / `CreateEncryptor` 有呼叫者，留著），並在類別 doc 說明「刻意沒有這個方法」與理由。<br>**但真正的危險門是 `Initialize(serializer, …)`，移除 factory 方法關不上它** —— 任何人 `new JsonPayloadSerializer()` 一樣做得到。因此在 `Initialize` 補 WARNING：那個參數不是「本部署用哪個 codec」，而是「**未宣告 codec 的請求被當成哪個 codec 讀**」，那是相容性常數；換掉它等於用既有 client 沒用過的 codec 去讀它們的 body，而失敗發生在伺服端反序列化時、離這個呼叫很遠
| **Z-6** ✅ 已修（2026-09-04） | `BusinessObject` 建構子 `isLocalCall = true` 預設（`src/Bee.Business/BusinessObject.cs:37` 與三個子類），**上輪明示遞延，本輪後果已具體化**：四處 defence-in-depth（`SystemBusinessObject.Plugin.cs:46-49,82-85`、`DeploymentAdmin.cs:43-46`、`Define.cs:361-366`）的註解都寫「a caller constructing the BO directly never passes through `ApiAccessValidator`」，而**直接建構走預設路徑時 `IsLocalCall == true`，四道守衛全數放行** —— 只有刻意寫 `false` 的呼叫端會被擋，那是最不需要擋的一種。`IsLocalCall == true` 解鎖的能力包含：任意鑄造 API key、`SetDeploymentAdmin`、`SaveDefine`、以及 `ApiAccessValidator.cs:39` 的 `if (context.IsLocalCall) return;`（**整個存取控制跳過**）。P0-1 證明這種漏傳在框架內部就會發生。<br>選項：移除四個預設值（破壞性，約 80 呼叫點含 `apps/`/`samples/`）；或**改預設為 `false`**（安全的那一邊，破壞性小得多）；或至少把四處守衛的註解改成描述實情。連帶 `BusinessObject.cs:70` 的 `public bool IsLocalCall { get; } = false;` 初始式與建構子預設相反，**讀起來像預設 false**<br>**已落地**：五個 BO 建構子的 `isLocalCall` 預設由 `true` 改為 `false`。<br>**影響範圍實測後比預期小很多**：`src/` / `apps/` / `samples/` / `tools/` **完全沒有直接建構 BO**，全部 120+ 個建構點都在 `tests/`，而 `BusinessObjectFactory` 一律顯式傳入 —— 這個預設值在框架內部從來沒被用到過，只作用於測試與**外部消費者**，而後者正是最不設防的那群。<br>**破壞面實測 28 筆測試**，全部集中在四個守衛保護的區域（API key 生命週期、deployment admin、deployment audit）—— 它們原本能過正是因為預設 `true` 讓守衛失效。修法是在 6 個建構點寫出 `isLocalCall: true`，把「我要走本機路徑」變成明講的而非預設撿到的。<br>**相容性**：原始碼行為變更（往安全那邊）；二進位上 optional 參數預設值在**呼叫端**編譯時烘入，既有已編譯組件維持舊行為，只有重編才拿到 `false`。PublicAPI 4 行 Shipped → Unshipped。<br>**閘門**：`LocalCallDefaultGateTests` 以反射掃**所有** `BusinessObject` 子類，任何建構子預設 `true` 就紅 —— 原本五個建構子各自宣告預設值，沒有任何機制要求一致。負向驗證做了兩層：第一次被 PublicAPI 閘門搶先擋下（改預設值等於改簽章），連同基準檔一起改之後才驗到這道閘門本身。
| **Z-7** ✅ 已修（2026-09-04） | `nuget-publish.yml` 的 build 清單與 pack 清單不對稱（`:44-61` vs `:66-83`）：`src/Bee.Expressions` **在 pack 清單、不在 build 清單**，而 pack 帶 `--no-build`。目前靠 `Bee.UI.Avalonia`/`Bee.Hosting` 的 `ProjectReference` 傳遞建到它才沒炸；那兩條參考一旦消失，**發版當下才會失敗**<br>**已落地**：**兩份手工清單合併為一份，build 與 pack 同一個 step**。`Bee.Expressions` 在 pack 清單、不在 build 清單，而 pack 帶 `--no-build` —— 目前只靠傳遞參考才沒炸，那兩條參考一旦消失就是**發版當下**失敗。<br>刻意不拆兩步：拆了就得把清單傳過去，而任何傳遞手段（檔案、env）都會再引入自己的失敗模式（第一版寫成檔案往返，`Out-File` 的 BOM 會讓第一個路徑失效，已自行否決）
| **Z-8** ✅ 已修（2026-09-04） | 公開文件連進 `.claude/`（`docs/api-method-reference.md:22,140,141` + `.zh-TW`、`docs/development-constraints{,.zh-TW}.md:6`、`adr-006:65`）。`rules/public-docs.md` 明列 `.claude/` 為非產品文件；其中 `adr-006:65` 指的是 `~/.claude/rules/code-style.md`（**使用者家目錄**，外部讀者永遠開不了）。`check-public-docs.sh` 只掃 `docs/plans/`，不涵蓋這個方向<br>**已落地**：五處指向 `.claude/` 的連結改指公開文件（`permission-authorization` / ADR-004 / ADR-044）；`adr-006` 那句指向**使用者家目錄**的路徑改為不帶路徑的敘述。<br>**並加進 `check-public-docs.sh` 第六道**：公開 markdown 不得指向 `.claude/`，排除 `CLAUDE.md` 自身（它就是 agent 規範）。前五道只掃 `docs/plans/`，這個方向從來沒被涵蓋 —— 這五處因此活到本次盤點。負向驗證通過
| **Z-9** ✅ 已修（2026-09-04） | CHANGELOG 破壞性節標題大小寫漂移：舊 10 版 `### Breaking Changes`，4.24.0/4.25.0/4.27.0 三版改用 `### Breaking changes`。中文版 13 版全為「破壞性變更」，一致。三版皆在基準後 → 回歸<br>**已落地**：根 CHANGELOG 3 處 + 明細檔 2 處的 `### Breaking changes` 統一為 `### Breaking Changes`
| **Z-10** ✅ 已修（2026-09-04） | `docs/plans/plan-property-grid-control.md:10-16` 把 `[DefaultValue]`（144 處）併入「純編輯器用途、零消費端」清單，**但它不是零消費端** —— `XmlSerializer` 依它省略等於預設值的成員。實證：`FormField.Type`/`ControlType`/`MaxLength` 三個屬性在 `tests/Define/FormSchema/AuditRule.FormSchema.xml` 的 `<FormField>` 中**完全不輸出**。動它會改變所有定義檔的序列化形狀。是 plan 不是公開文件，但這句話會直接誤導實作者<br>**已落地**：`plan-property-grid-control.md` 補上警告：`[DefaultValue]` **有消費端**，`XmlSerializer` 依它省略「等於預設值」的成員。實證確認 `FormField` 的 `Type` / `ControlType` / `MaxLength` 在 `AuditRule.FormSchema.xml` 的 `<FormField>` 中完全不輸出。原文把它併進「零消費端」那句會直接誤導實作者
| **Z-11** ✅ 已修（2026-09-04） | `ApiSessionContext.Ambient` 逃生門：多使用者 host 若忘記註冊 scoped context 而落到 `Ambient`，共用的 `NextSequence()` 會讓單一 session 的序號出現大間隔；同一 session 自己的亂序請求若差距 ≥ `WindowSize`(64) 會被誤判為重放。`Bee.Web.Blazor.Server` 已正確 `AddScoped`，外部 host 自行接線時是陷阱<br>**已落地**：補 WARNING：落到 `Ambient` 不會大聲失敗，而是**表現為沒人解釋得了的重放拒絕** —— 共用計數器讓單一 session 的序號出現大間隔，其自身亂序請求差距夠大就被判為重放。並點名 `Bee.Web.Blazor.Server` 已 per-circuit 註冊，自行接線的 host 要照做
| **Z-12** ✅ 已修（2026-09-04） | `DbDefineStorage.cs:89-91` 的 `_connectionManager ??= Resolve<T>()` 無同步，**目前安全只因為解析出來的也是 singleton**。若日後改成 transient，會靜默產生 per-race 實例且無任何徵兆<br>**已落地**：補 NOTE：`??=` 無同步**只因為解析出來的是 singleton** —— 那是註冊方式的性質、不是這段程式碼的性質。改成 transient 會靜默產生 per-race 實例且無徵兆
...
| **Z-14** ✅ 已修（2026-09-04） | `ApiAccessValidator.cs:46` 的死分支：`if (attr.ProtectionLevel == LocalOnly && !context.IsLocalCall)` —— 上方 `:39` 已 `if (context.IsLocalCall) return;`，故後半恆真。無害，但讀者會以為它在做事<br>**已落地**：移除恆真的 `&& !context.IsLocalCall`（上方已對 local call 提早返回），並註明原本那半讀起來像在做判斷
...
| **Z-16** ✅ 已修（2026-09-04） | `BeeServiceProviderExtensions`（`src/Bee.Definition/`）是潛在 CS0121 地雷：以 `internal` 提供 `GetService<T>`/`GetRequiredService<T>`，經 `InternalsVisibleTo` 被 `Bee.Business` 約 50 個呼叫點消費。目前只有 `FormPluginRunner.cs` 一個檔 `using Microsoft.Extensions.DependencyInjection` 且該檔沒有這類呼叫，所以編得過。**任何人在該檔（或未來新檔）同時引入這兩者並呼叫，就會是 CS0121**。XML doc 已寫撞名理由，但沒寫這個觸發條件<br>**已落地**：補 WARNING 與**觸發條件**：`internal` 只避開套件邊界的撞名，`Bee.Business` 內部才是陷阱 —— 任何檔案同時 `using Microsoft.Extensions.DependencyInjection` 並呼叫 `GetService<T>` 就是 CS0121。目前編得過是巧合（唯一帶該 using 的 `FormPluginRunner.cs` 剛好沒這類呼叫），不是設計
