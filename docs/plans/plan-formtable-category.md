# 計畫：FormTable 加上 Category 欄位，驅動 TableSchema 自動分類
...
CLAUDE.md 中明示「FormSchema 為定義中樞，同時驅動 UI、資料庫與驗證規則」；目前 Category 是這條原則少數的例外。

## 目標

讓 FormTable 自帶 Category 歸屬，使「FormSchema 即可決定 TableSchema 的存放分類」，並讓 `DbCategorySettings.xml` 從手動同步表退化為「Category 主檔（DisplayName / 連線字串對應）」。

具體：
1. `FormTable` 新增 `Category` 屬性（`XmlAttribute`），預設 `"common"`
2. `TableSchemaGenerator.Generate(...)` 行為不變（仍只產 TableSchema 內容），但提供新 helper `TableSchemaGenerator.GetCategory(formTable)` 給呼叫端決定落點
3. `LocalDefineAccess.SaveFormSchema(...)` 在落地 FormSchema 時，依 FormTable 的 Category 同步維護 `DbCategorySettings.xml` 中的 `<TableItem>` 清單（新增/移動/刪除）
4. `DbCategorySettings` 改為「Category 主檔」：仍保留 `Id` / `DisplayName`，但 `<Tables>` 子節點轉為由 SaveFormSchema 自動寫回，不再由人工編輯
5. 既有 fixture 與測試遷移，所有現存 FormTable 預設標 `Category="common"`（與目前 fixture 既有歸類一致）

## 設計決策（需使用者確認）

下列三點影響範圍較大，先列出待確認，再動手實作：

### D1. Category 放在 `FormTable` 還是 `FormSchema`？

- **方案 A（建議）**：放 `FormTable`。每個 FormTable 可獨立歸類，符合「Master 與 Detail 可能放不同 DB」的彈性
- 方案 B：放 `FormSchema`。整個 schema 一個 Category，較簡單但會犧牲 Master/Detail 跨 DB 的可能性

> 若目前實務上不存在 Master/Detail 跨 Category 的情境，B 也可接受；待確認後再定。

### D2. 預設值

- **方案 A（建議）**：`Category` 預設為 `"common"`，與既有 fixture 與 `DbCategorySettings.xml` 中的 `Id="common"` 一致
...
- 新增 `[XmlAttribute] public string Category { get; set; } = "common";`
...
- 新增 `public static string GetCategory(FormTable formTable)` helper（單純回傳 `formTable.Category`，未來若有 fallback 邏輯可集中於此）
...
- 落檔 FormSchema 後，迭代 `formSchema.Tables`，將「TableName + DisplayName + Category」收集為一份「期望落點」
- 讀出當前 `DbCategorySettings`，比對：
  - 新表：在對應 `DbCategory.Tables` 加上 `<TableItem>`
  - 移動：從舊 Category 移除，加到新 Category
  - 刪除（若 FormTable 已不存在）：從原 Category 移除
  - 目標 Category 不存在時：丟 `InvalidOperationException`，提示先在 `DbCategorySettings.xml` 建立 Category 主檔
...
- `tests/Define/FormSchema/*.FormSchema.xml`：所有 FormTable 補 `Category="common"`（或保留預設不寫，若預設機制可省略 attribute）
...
- 新增測試（`Bee.ObjectCaching.UnitTests`）：
  ...
  - `SaveFormSchema_MovesTableItem_WhenCategoryChanged`
  - `SaveFormSchema_RemovesTableItem_WhenFormTableDeleted`
  - `SaveFormSchema_ThrowsWhenCategoryNotDefined`
  - 全部以 `using var temp = new TempDefinePath();` 包覆（依 `rules/testing.md` 共享 fixture 隔離規定）

### D. 文件

- `docs/development-cookbook.md`：FormSchema 章節補一段 Category 屬性與自動同步行為
- `docs/architecture-overview.md`：「DbCategory 由人工 XML 維護」改為「FormTable.Category 為事實來源，DbCategorySettings 為索引快照」
- ADR：考慮新增 `docs/adr/NNNN-formtable-category-as-source-of-truth.md`，記錄這個決策的背景與遷移策略

## 步驟

1. 取得使用者對 D1 / D2 / D3 的確認
2. 實作 A（FormTable.Category + TableSchemaGenerator helper）
3. 補單元測試（純邏輯部分）：FormTable 的 Category 序列化往返、預設值
...
- 是否需要在 FormTable 上額外加 `ConnectionId` 之類的細節？目前 `DbCategory` 已隱含對應一組連線設定，FormTable 知道 Category 即可推到連線；除非有「同 Category 不同連線」的需求，否則不加。
