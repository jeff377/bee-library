...
| 6 | bee-jsonrpc-sample | `samples/JsonRpcClient` + `samples/JsonRpcServer` + `samples/Custom.Business` + `samples/Custom.Contracts` + `samples/Define` | Sample 專案 |
...
3. **bee-jsonrpc-sample** — 所有相依套件已在 NuGet，可直接移出
4. **Bee.Api.AspNet** — 僅依賴已在 NuGet 的 Bee.Api.Core，可直接移出
5. **Bee.UI.Core** — 僅依賴已在 NuGet 的 Bee.Connect，可同步移出
6. **Bee.UI.WinForms** — 等 Bee.UI.Core 發布 NuGet 後移出
