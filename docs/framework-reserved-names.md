...
- **To get the base XML to start customising from** — three options, in order of typical preference:
    ...
    - **CLI** (recommended for CI / setup scripts): install once with `dotnet tool install -g Bee.Cli`, then `dotnet bee defines materialize --path ./Define` — thin shell over the same API. Run `dotnet bee defines list` to see every embedded file, `dotnet bee defines materialize --filter TableSchema/` to materialise a subset.
