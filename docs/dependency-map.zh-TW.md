...
- **Bee.UI.Maui** 為 MAUI 跨平台控制項套件（iOS / Android / macOS / Windows）；目前為 **placeholder 骨架**（plain `net10.0` class library，僅 reference `Bee.UI.Core`），第一個實際控制項加入時 csproj 將改寫為 MAUI multi-target 形式。詳見 `docs/plans/plan-add-bee-ui-maui.md`。
