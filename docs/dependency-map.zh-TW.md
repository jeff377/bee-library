...
本文件以視覺化方式呈現 Bee.NET 框架中 15 個 `src/` 專案之間的相依關係。
...
- **Bee.UI.Core** 為跨平台 UI 共通層（`ClientInfo` / `IEndpointStorage` / `IUIViewService` / `VersionInfo`），供桌面端（WinForms / WPF / Avalonia）、Web 前端、未來 MAUI 等共用 client-side 連線狀態與 endpoint 持久化邏輯；不含任何平台專屬 UI 程式碼，只依 `Bee.Api.Client`。
