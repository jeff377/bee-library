...
- **Bee.UI.Maui** is the MAUI cross-platform control library (iOS / Android / macOS / Windows). It is currently a **placeholder skeleton** (plain `net10.0` class library that only references `Bee.UI.Core`); when the first concrete control lands, the csproj will be rewritten to MAUI multi-target form. See `docs/plans/plan-add-bee-ui-maui.md`.
