...
`docs/` 存放 Bee.NET 框架對外公開的開發者文件。以下所列文件皆有繁體中文與英文兩個版本，檔名相同：繁體中文版在 `docs/zh-TW/`，英文版在 `docs/en/`。繁體中文為源文件，其他語言皆為譯本。
...
| 文件 | 類型 | 篇幅 | 說明 |
|------|------|------|------|
| [快速上手](getting-started.md) | 教學 | 中 | 從零建出第一個 Bee.NET 後端：套件、`DefinePath`、DI 接線、第一張表單與商業物件，再由用戶端呼叫 |
...
| 文件 | 類型 | 篇幅 | 說明 |
|------|------|------|------|
| [FormSchema 驅動的資料庫存取](formschema-data-access.md) | 概念 | 中 | Bee.Db 如何以 FormSchema 為單位動態產生 SQL，以及它為何不是 ORM |
