...
| `CreateApiKey` | LocalOnly | Authenticated | 發放 API 金鑰，完整明文金鑰**只回傳一次** —— 伺服端只存雜湊，無法再次顯示。鑄造憑證屬部署期作業，遠端呼叫一律拒絕。 |
...
| `GetDbAnomalyLog` | Encrypted | Authenticated | `st_log_anomaly_db` 標頭的過濾分頁清單（時間 / databaseId / anomaly-kind）。屬跨公司的基礎設施檢視（`st_log_anomaly_db` 無公司維度）。回傳標頭 `DataTable` + `PagingInfo`。 |
...
| `GetDbAnomalySummary` | Encrypted | Authenticated | 依 `anomaly_kind` 彙總 DB 異常筆數（可帶時間窗）。跨公司基礎設施摘要。回傳聚合 `DataTable`（`anomaly_kind` / `event_count`），無分頁。 |
