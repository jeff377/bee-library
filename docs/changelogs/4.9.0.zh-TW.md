...
- **`GridControl` — 對應 `LayoutGrid` 的 grid 控件** — 組合式控件（內建圖示工具列 + 公開為 `InnerGrid` 的內部 `DataGrid`），支援兩種顯式綁定（`FormDataObject` 明細表依 `TableName` 解析、或列表模式直接給 `DataTable`）與 `FormScope` ambient 綁定。依 `LayoutColumn.ControlType` 提供 in-cell 編輯（popup 型欄位採點擊置換呈現——見 [ADR-021](../adr/adr-021-avalonia-datagrid-editing-strategy.md)）、`AllowActions` 增刪列、host 端以單一 `AllowEdit` 屬性控制。`DynamicForm` 補上 `FormLayout.Details` 渲染（每個 `LayoutGrid` 一組 caption + 綁定 `GridControl`）。
