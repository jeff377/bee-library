...
- **JSON-RPC 前端整合指引** — 新增 [`docs/jsonrpc-frontend-integration.md`](../jsonrpc-frontend-integration.md)（中英雙語），涵蓋 wire format、headers、認證流程、可呼叫方法清單、`JsonRpcErrorCode` 對應表與 TypeScript wrapper snippet，供 React / Vue / Angular / vanilla JS 前端參照。
