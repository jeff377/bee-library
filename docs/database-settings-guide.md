...
業務程式對部署形態無感：永遠透過 `BackendInfo.GetDatabaseItem(databaseId)` 取連線。差異只在業務層怎麼決定要傳哪個 `databaseId`：
...
- **加密方式**：AES-CBC-HMAC（金鑰來源 `BackendInfo.ConfigEncryptionKey`）
- **儲存格式**：`enc:` + Base64 編碼的密文
- **時機**：
  - 序列化前（`BeforeSerialize`）：將未加密的 Password 自動加密
  - 反序列化後（`AfterDeserialize`）：將 `enc:` 開頭的 Password 自動解密
- **行為**：若 `BackendInfo.ConfigEncryptionKey` 未設定，跳過加解密（明文儲存，僅限開發環境）
...
兩個 settings 都透過 `BackendInfo.DefineAccess` 存取：
...
`DefineAccess` 由初始化階段注入 `LocalDefineAccess`（檔案系統）或 `RemoteDefineAccess`（透過 API 取得）。

### 4.2 快取機制

兩個 settings 都由 [`CacheContainer`](../src/Bee.ObjectCaching/CacheContainer.cs) 集中持有，採 `Lazy<T>` 延遲建立：
...
```csharp
// 取單一連線項目（內部用 DatabaseSettings.Items[databaseId]）
...
`BackendInfo.GetDatabaseItem` 在找不到 Id 時拋 `KeyNotFoundException`，呼叫端可依此判斷未知連線。
...
路徑解析：[`DefinePathInfo.GetTableSchemaFilePath(categoryId, tableName)`](../src/Bee.Definition/DefinePathInfo.cs)。
...
無論底層是哪種狀況，業務端程式碼都是同一個入口 `BackendInfo.GetDatabaseItem(databaseId)`，差異只在「依當前情境推導 databaseId 字串」這一步：
...
兩個設定檔皆位於 `BackendInfo.DefinePath` 根目錄：
