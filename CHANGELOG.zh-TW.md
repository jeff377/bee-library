...
- `Bee.Base`：`StringUtilities.IsEmpty(object?)` 與 `IsNotEmpty(object?)` 改名為 `IsEmptyText` / `IsNotEmptyText`。它們是先把值轉成字串再判斷文字，與 `ValueUtilities.IsEmpty(object)` 問的不是同一個問題——兩者對 `Guid.Empty` 與 `DateTime.MinValue` 的結論相反，而舊名稱看不出呼叫端指的是哪一個。字串多載維持原名。框架內受影響的呼叫點只有一處。
