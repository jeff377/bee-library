...
## BO 介面是 BO-to-BO 解耦層，純 API 方法不放介面
...
**只透過 `JsonRpcExecutor.Execute` 派發給 client、沒有跨 BO 消費者的方法**（`Ping`、`Login`、
`GetFormSchema`、`GetFormLayout`、`GetLanguage`）在實體類別上仍是 `public` + `[ApiAccessControl]`，
executor 透過反射照樣派得到，但**不放介面**。

新增 BO method 時先問：「會不會有另一個 BO 透過 `_ctx.BoFactory.CreateXxxBO(...)` 拿到後呼叫它？」
是 → 放介面；否 → 不放。介面爆成「所有 public 方法集合」就失去意義，也增加 host 端客製化負擔。
