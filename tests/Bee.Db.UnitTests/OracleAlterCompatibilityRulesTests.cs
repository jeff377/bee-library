...
{
    /// <summary>
    /// 對應 <c>OracleAlterCompatibilityRules</c>（internal static class）的純規則測試。
    /// 透過 reflection 呼叫，避免修改 production 可見性。
    /// </summary>
    public class OracleAlterCompatibilityRulesTests : IClassFixture<SharedDbFixture>
    {
        public OracleAlterCompatibilityRulesTests(SharedDbFixture _) { }

        private static readonly Type s_rulesType =
            typeof(OracleDialectFactory).Assembly.GetType("Bee.Db.Providers.Oracle.OracleAlterCompatibilityRules", throwOnError: true)!;

        private static ChangeExecutionKind GetKindForTypeChange(FieldDbType from, FieldDbType to)
        {
            var method = s_rulesType.GetMethod("GetKindForTypeChange", BindingFlags.Public | BindingFlags.Static)!;
            return (ChangeExecutionKind)method.Invoke(null, new object[] { from, to })!;
        }

        private static bool IsNarrowing(DbField oldField, DbField newField)
        {
            var method = s_rulesType.GetMethod("IsNarrowing", BindingFlags.Public | BindingFlags.Static)!;
            return (bool)method.Invoke(null, new object[] { oldField, newField })!;
        }
        ...
        [DisplayName("Oracle GetKindForTypeChange:同型別應為 Alter")]
        public void GetKindForTypeChange_SameType_ReturnsAlter(FieldDbType from, FieldDbType to)
        {
            Assert.Equal(ChangeExecutionKind.Alter, GetKindForTypeChange(from, to));
        }

        [Fact]
        [DisplayName("Oracle GetKindForTypeChange:AutoIncrement→AutoIncrement 同型別應為 Alter")]
        public void GetKindForTypeChange_AutoIncrementToSame_ReturnsAlter()
        {
            Assert.Equal(ChangeExecutionKind.Alter, GetKindForTypeChange(FieldDbType.AutoIncrement, FieldDbType.AutoIncrement));
        }
        ...
        [DisplayName("Oracle GetKindForTypeChange:同 family 應為 Alter")]
        ...
        [DisplayName("Oracle GetKindForTypeChange:跨 family 應為 Rebuild")]
        public void GetKindForTypeChange_CrossFamily_ReturnsRebuild(FieldDbType from, FieldDbType to)
        {
            Assert.Equal(ChangeExecutionKind.Rebuild, GetKindForTypeChange(from, to));
        }
        ...
        [DisplayName("Oracle GetKindForTypeChange:AutoIncrement(IDENTITY)狀態變更應為 Rebuild")]
        ...
        [DisplayName("Oracle GetKindForTypeChange:Unknown 應為 NotSupported")]
        public void GetKindForTypeChange_UnknownType_ReturnsNotSupported(FieldDbType from, FieldDbType to)
        {
            Assert.Equal(ChangeExecutionKind.NotSupported, GetKindForTypeChange(from, to));
        }

        [Fact]
        [DisplayName("Oracle IsNarrowing:String 長度縮小應判定為 narrowing")]
        public void IsNarrowing_StringLengthReduced_ReturnsTrue()
        {
            ...
            Assert.True(IsNarrowing(oldField, newField));
        }

        [Fact]
        [DisplayName("Oracle IsNarrowing:String 長度放大不是 narrowing")]
        public void IsNarrowing_StringLengthIncreased_ReturnsFalse()
        {
            ...
            Assert.False(IsNarrowing(oldField, newField));
        }

        [Fact]
        [DisplayName("Oracle IsNarrowing:Text(CLOB)轉 String(有長度上限)應判定為 narrowing")]
        ...
        [Fact]
        [DisplayName("Oracle IsNarrowing:String 轉 Text(CLOB)不是 narrowing")]
        ...
        [DisplayName("Oracle IsNarrowing:數值型縮小應判定為 narrowing")]
        ...
        [InlineData(FieldDbType.Short, FieldDbType.Integer)]
        [InlineData(FieldDbType.Integer, FieldDbType.Long)]
        [DisplayName("Oracle IsNarrowing:數值型放大不是 narrowing")]
        ...
        [Fact]
        [DisplayName("Oracle IsNarrowing:Decimal precision 縮小應判定為 narrowing")]
        ...
        [Fact]
        [DisplayName("Oracle IsNarrowing:Decimal scale 縮小應判定為 narrowing")]
        ...
        [Fact]
        [DisplayName("Oracle IsNarrowing:DateTime 轉 Date 應判定為 narrowing(時間精度遺失)")]
        ...
        [Fact]
        [DisplayName("Oracle IsNarrowing:Date 轉 DateTime 不是 narrowing")]
        ...
        [Fact]
        [DisplayName("Oracle IsNarrowing:跨 family 不是 narrowing(由 GetKindForTypeChange 處理)")]
        public void IsNarrowing_CrossFamily_ReturnsFalse()
        {
            var oldField = new DbField("v", "V", FieldDbType.Integer);
