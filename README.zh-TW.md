...
| 套件 | 安裝 | 說明 |
|---|---|---|
| **Bee.Cli** | `dotnet tool install -g Bee.Cli` | 框架 CLI，命令名 `dotnet bee`。本版 ship 出 `defines` subcommand group，用於 materialize / list 框架預設定義檔（`st_*` TableSchema、框架預設 FormSchema / FormLayout / Language、SystemSettings / DatabaseSettings template）。用於新消費者從 `Bee.Definition.dll` 內 embedded 資源 bootstrap `DefinePath`。 |
