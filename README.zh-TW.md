...
| **Bee.UI.Avalonia.dll** | Avalonia 桌面控制項套件（Windows / macOS / Linux），提供 FormSchema 驅動控制項（`DynamicForm` + `DynamicGrid` + `FormView` + `FormDataObject`）與檔案後端 `FileEndpointStorage`。單一 `net10.0` TFM；下限版本鎖在 Avalonia 12.0.0 + DataGrid 12.0.0。 |
