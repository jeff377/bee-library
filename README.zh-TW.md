...
Bee.NET Framework 是一套採用 **N-Tier + Clean Architecture + MVVM** 混合模式的企業資訊系統開發框架，以**定義導向架構（Definition-Driven Architecture）**為核心，以 `FormDefine` 作為系統唯一定義來源（Single Source of Truth），統一驅動 UI 配置、資料表結構與業務驗證規則。
...
- **定義導向架構（Definition-Driven Architecture）**：以 `FormDefine` 作為系統唯一定義來源，自動推導 UI 配置（`FormLayout`）、資料表結構（`DbTable`）與驗證規則，定義一處即全層同步。
...
- **開發加速器**：透過可重用基底類別與 FormDefine 驅動的 CRUD，大幅減少重複程式碼。
...
| **Bee.Define.dll** | 定義系統結構化資料，包含 FormDefine、欄位結構描述與版面配置。 |
...
| **Bee.Cache.dll** | 執行階段快取 FormDefine 定義資料與衍生資料，提升系統效能。 |
| **Bee.Db.dll** | 封裝資料庫操作邏輯，支援動態 SQL 命令產生與連線綁定。 |
| **Bee.Repository.dll** | 提供共用的 Repository 基底類別與 FormDefine 驅動的資料存取機制。 |
...
| 組件名稱 | 說明 |
|---|---|
| **Bee.Connect.dll** | 提供連接器機制，支援近端與遠端呼叫後端 Business Object。 |
